...
#Region "Funciones"
	...
	Public Overrides Function InsertarRegistro(ByVal pvo_Registro As EntBase) As Integer
		...
		Dim vlo_RegistroInterno As EntTcmAutor
		Dim vln_Resultado As Integer

		Try
			vln_Resultado = -1
			vlo_RegistroInterno = CType(pvo_Registro, EntTcmAutor)
			...
			vlc_Sentencia = "prI_TCM_AUTOR"

			vlo_Conexion.SetParameter("pvc_IdPersonal", OracleDbType.Varchar2, vlo_RegistroInterno.IdPersonal)
			vlo_Conexion.SetParameter("pvc_Nombre", OracleDbType.Varchar2, vlo_RegistroInterno.Nombre)
			vlo_Conexion.SetParameter("pvc_PrimerApellido", OracleDbType.Varchar2, vlo_RegistroInterno.PrimerApellido)
			vlo_Conexion.SetParameter("pvc_SegundoApellido", OracleDbType.Varchar2, vlo_RegistroInterno.SegundoApellido)
			vlo_Conexion.SetParameter("pvd_FechaHoraNacimiento", OracleDbType.Date, vlo_RegistroInterno.FechaHoraNacimiento)
	...
	''' <summary>
	''' Permite modificar un registro en la tabla TCM_AUTOR
	''' </summary>
	''' <param name="pvo_Registro">Entidad a procesar</param>
	''' <returns>Mayor a cero: El proceso ha sido exitoso. Menor a cero: El proceso ha fallado</returns>
	''' <author>Generador de Código basado en objetos Oracle</author>
	...
	Public Overrides Function ObtenerRegistro(ByVal pvc_Condicion As String) As EntBase
		...
		Dim vlo_Resultado As New EntTcmAutor

		Try
			vlo_DsDatos = ListarRegistros(pvc_Condicion, String.Empty, False, 0, 0)
			If vlo_DsDatos IsNot Nothing AndAlso vlo_DsDatos.Tables(0).Rows.Count > 0 Then
				vlo_MapeoEntidad = New List(Of MapeoSimple)
				vlo_MapeoEntidad.Add(New MapeoSimple(TCM_AUTOR.ID_PERSONAL, "IdPersonal"))
				vlo_MapeoEntidad.Add(New MapeoSimple(TCM_AUTOR.NOMBRE, "Nombre"))
				vlo_MapeoEntidad.Add(New MapeoSimple(TCM_AUTOR.PRIMER_APELLIDO, "PrimerApellido"))
				vlo_MapeoEntidad.Add(New MapeoSimple(TCM_AUTOR.SEGUNDO_APELLIDO, "SegundoApellido"))
				vlo_MapeoEntidad.Add(New MapeoSimple(TCM_AUTOR.FECHA_HORA_NACIMIENTO, "FechaHoraNacimiento"))
				vlo_MapeoEntidad.Add(New MapeoSimple(TCM_AUTOR.ESTADO, "Estado"))
				vlo_MapeoEntidad.Add(New MapeoSimple(TCM_AUTOR.TIME_STAMP, "TimeStamp"))
				vlo_MapeoEntidad.Add(New MapeoSimple(TCM_AUTOR.USUARIO, "Usuario"))

				vlo_Resultado = vlo_DsDatos.Tables(0).Rows(0).ToEntity(Of EntTcmAutor)(vlo_MapeoEntidad)
	...
	Public Overrides Function ListarRegistros(pvc_Condicion As String, pvc_Orden As String, pvb_Paginar As Boolean, pvn_NumeroPagina As Integer, pvn_TamanoPagina As Integer) As DataSet
		...
		Try
			...
			vlo_DsDatos = Me.EjecutarVista(vlo_Conexion, TCM_AUTOR.Name, "V_TCM_AUTOR", pvc_Condicion, pvc_Orden, pvb_Paginar, pvn_NumeroPagina, pvn_TamanoPagina)
	...
	''' <creationDate>4/5/2017 1:54:32 p. m.</creationDate>
	''' <changeLog></changeLog>
	Public Function ObtenerDatosPaginacionVTcmAutor(pvc_Condicion As String, pvc_Orden As String, pvn_TamanoPagina As Integer) As EntDatosPaginacion
		Dim vlo_Conexion As DbBase
		Dim vlb_DisposeConexion As Boolean
		...
		Try
			...
			vlo_Resultado = Me.ObtenerDatosPaginacionVista(vlo_Conexion, "V_TCM_AUTOR", pvc_Condicion, pvc_Orden, pvn_TamanoPagina)
	...
	Public Overrides Function ListarRegistrosLista(pvc_Condicion As String, pvc_Orden As String, pvb_Paginar As Boolean, pvn_NumeroPagina As Integer, pvn_TamanoPagina As Integer) As DataSet
		...
		Try
			...
			vlo_DsDatos = Me.EjecutarVista(vlo_Conexion, TCM_AUTOR.Name, "V_TCM_AUTORLst", pvc_Condicion, pvc_Orden, pvb_Paginar, pvn_NumeroPagina, pvn_TamanoPagina)
	...
	''' <creationDate>4/5/2017 1:54:32 p. m.</creationDate>
	''' <changeLog></changeLog>
	Public Function ObtenerDatosPaginacionVTcmAutorlst(pvc_Condicion As String, pvc_Orden As String, pvn_TamanoPagina As Integer) As EntDatosPaginacion
		Dim vlo_Conexion As DbBase
		Dim vlb_DisposeConexion As Boolean
		Dim vlo_Resultado As EntDatosPaginacion
		...
		Try
			...
			vlo_Resultado = Me.ObtenerDatosPaginacionVista(vlo_Conexion, "V_TCM_AUTORLst", pvc_Condicion, pvc_Orden, pvn_TamanoPagina)
	...
	''' <creationDate>4/5/2017 1:54:32 p. m.</creationDate>
	''' <changeLog></changeLog>
	Public Function ListarFcTcAutoresDelLibro(pvc_Condicion As String, pvc_Orden As String, pvb_Paginar As Boolean, pvn_NumeroPagina As Integer, pvn_TamanoPagina As Integer, pvc_PvcIsbn As String) As DataSet
		Dim vlo_Conexion As DbBase
		Dim vlb_DisposeConexion As Boolean
		Dim vlc_Sentencia As String
		Dim vlo_DsDatos As DataSet
		...
		Try
			...
			vlc_Sentencia = "FC_TC_AUTORES_DEL_LIBRO(:pvc_PvcIsbn)"
			vlo_Conexion.SetParameter("pvc_PvcIsbn", OracleDbType.Varchar2, pvc_PvcIsbn)

			vlo_DsDatos = EjecutarFuncionRetornoTabla(vlo_Conexion, "FC_TC_AUTORES_DEL_LIBRO", vlc_Sentencia, TP_TC_DATOS_AUTOR.Name, pvc_Condicion, pvc_Orden, pvb_Paginar, pvn_NumeroPagina, pvn_TamanoPagina)
	...
	''' <creationDate>4/5/2017 1:54:32 p. m.</creationDate>
	''' <changeLog></changeLog>
	Public Function ObtenerDatosPaginacionFcTcAutoresDelLibro(pvc_Condicion As String, pvc_Orden As String, pvn_TamanoPagina As Integer, pvc_PvcIsbn As String) As EntDatosPaginacion
		Dim vlo_Conexion As DbBase
		Dim vlb_DisposeConexion As Boolean
		Dim vlc_Sentencia As String
		Dim vlo_Resultado As EntDatosPaginacion
		...
		Try
			...
			vlo_Resultado = ObtenerDatosPaginacionFuncionRetornoTabla(vlo_Conexion, "FC_TC_AUTORES_DEL_LIBRO", vlc_Sentencia, pvc_Condicion, pvc_Orden, pvn_TamanoPagina)
	...
	''' <creationDate>4/5/2017 1:54:32 p. m.</creationDate>
	''' <changeLog></changeLog>
	Public Function ListarFcTcAutoresNoAsignadosLib(pvc_Condicion As String, pvc_Orden As String, pvb_Paginar As Boolean, pvn_NumeroPagina As Integer, pvn_TamanoPagina As Integer, pvc_PvcIsbn As String) As DataSet
		Dim vlo_Conexion As DbBase
		Dim vlb_DisposeConexion As Boolean
		Dim vlc_Sentencia As String
		Dim vlo_DsDatos As DataSet
		...
		Try
			...
			vlc_Sentencia = "FC_TC_AUTORES_NO_ASIGNADOS_LIB(:pvc_PvcIsbn)"
			vlo_Conexion.SetParameter("pvc_PvcIsbn", OracleDbType.Varchar2, pvc_PvcIsbn)

			vlo_DsDatos = EjecutarFuncionRetornoTabla(vlo_Conexion, "FC_TC_AUTORES_NO_ASIGNADOS_LIB", vlc_Sentencia, TP_TC_DATOS_AUTOR.Name, pvc_Condicion, pvc_Orden, pvb_Paginar, pvn_NumeroPagina, pvn_TamanoPagina)
	...
	''' <creationDate>4/5/2017 1:54:32 p. m.</creationDate>
	''' <changeLog></changeLog>
	Public Function ObtenerDatosPaginacionFcTcAutoresNoAsignadosLib(pvc_Condicion As String, pvc_Orden As String, pvn_TamanoPagina As Integer, pvc_PvcIsbn As String) As EntDatosPaginacion
		Dim vlo_Conexion As DbBase
		Dim vlb_DisposeConexion As Boolean
		Dim vlc_Sentencia As String
		Dim vlo_Resultado As EntDatosPaginacion
		...
		Try
			...
			vlo_Resultado = ObtenerDatosPaginacionFuncionRetornoTabla(vlo_Conexion, "FC_TC_AUTORES_NO_ASIGNADOS_LIB", vlc_Sentencia, pvc_Condicion, pvc_Orden, pvn_TamanoPagina)
