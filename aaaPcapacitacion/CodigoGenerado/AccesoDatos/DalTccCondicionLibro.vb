...
Namespace TallerCapacitacion.AccesoDatos.Catalogos
	Public Class DalTccCondicionLibro
		Inherits DalBase
...
#Region "Funciones"
	...
	Public Overrides Function InsertarRegistro(ByVal pvo_Registro As EntBase) As Integer
		...
		Dim vlo_RegistroInterno As EntTccCondicionLibro
		Dim vln_Resultado As Integer

		Try
			vln_Resultado = -1
			vlo_RegistroInterno = CType(pvo_Registro, EntTccCondicionLibro)
			...
			vlc_Sentencia = "prI_TCC_CONDICION_LIBRO"
	...
	Public Overrides Function BorrarRegistro(ByVal pvo_Registro As EntBase) As Integer
		...
		Try
			...
			vlc_Sentencia = "prD_TCC_CONDICION_LIBRO"
	...
	''' <summary>
	''' Permite modificar un registro en la tabla TCC_CONDICION_LIBRO
	''' </summary>
	''' <param name="pvo_Registro">Entidad a procesar</param>
	''' <returns>Mayor a cero: El proceso ha sido exitoso. Menor a cero: El proceso ha fallado</returns>
	''' <author>Generador de Código basado en objetos Oracle</author>
	...
	Public Overrides Function ModificarRegistro(ByVal pvo_Registro As EntBase) As Integer
		...
		Try
			...
			vlc_Sentencia = "prU_TCC_CONDICION_LIBRO"
	...
	Public Overrides Function ObtenerRegistro(ByVal pvc_Condicion As String) As EntBase
		...
		Dim vlo_Resultado As New EntTccCondicionLibro

		Try
			vlo_DsDatos = ListarRegistros(pvc_Condicion, String.Empty, False, 0, 0)
			If vlo_DsDatos IsNot Nothing AndAlso vlo_DsDatos.Tables(0).Rows.Count > 0 Then
				vlo_MapeoEntidad = New List(Of MapeoSimple)
				vlo_MapeoEntidad.Add(New MapeoSimple(TCC_CONDICION_LIBRO.CONDICION_LIBRO, "CondicionLibro"))
				vlo_MapeoEntidad.Add(New MapeoSimple(TCC_CONDICION_LIBRO.DESCRIPCION, "Descripcion"))

				vlo_Resultado = vlo_DsDatos.Tables(0).Rows(0).ToEntity(Of EntTccCondicionLibro)(vlo_MapeoEntidad)
	...
	Public Overrides Function ListarRegistros(pvc_Condicion As String, pvc_Orden As String, pvb_Paginar As Boolean, pvn_NumeroPagina As Integer, pvn_TamanoPagina As Integer) As DataSet
		...
		Try
			...
			vlo_DsDatos = Me.EjecutarVista(vlo_Conexion, TCC_CONDICION_LIBRO.Name, "V_TCC_CONDICION_LIBRO", pvc_Condicion, pvc_Orden, pvb_Paginar, pvn_NumeroPagina, pvn_TamanoPagina)
	...
	''' <creationDate>4/5/2017 4:34:41 p. m.</creationDate>
	''' <changeLog></changeLog>
	Public Function ObtenerDatosPaginacionVTccCondicionLibro(pvc_Condicion As String, pvc_Orden As String, pvn_TamanoPagina As Integer) As EntDatosPaginacion
		Dim vlo_Conexion As DbBase
		Dim vlb_DisposeConexion As Boolean
		...
		Try
			...
			vlo_Resultado = Me.ObtenerDatosPaginacionVista(vlo_Conexion, "V_TCC_CONDICION_LIBRO", pvc_Condicion, pvc_Orden, pvn_TamanoPagina)
	...
	Public Overrides Function ListarRegistrosLista(pvc_Condicion As String, pvc_Orden As String, pvb_Paginar As Boolean, pvn_NumeroPagina As Integer, pvn_TamanoPagina As Integer) As DataSet
		...
		Try
			...
			vlo_DsDatos = Me.EjecutarVista(vlo_Conexion, TCC_CONDICION_LIBRO.Name, "V_TCC_CONDICION_LIBROLst", pvc_Condicion, pvc_Orden, pvb_Paginar, pvn_NumeroPagina, pvn_TamanoPagina)
	...
	''' <creationDate>4/5/2017 4:34:41 p. m.</creationDate>
	''' <changeLog></changeLog>
	Public Function ObtenerDatosPaginacionVTccCondicionLibrolst(pvc_Condicion As String, pvc_Orden As String, pvn_TamanoPagina As Integer) As EntDatosPaginacion
		Dim vlo_Conexion As DbBase
		Dim vlb_DisposeConexion As Boolean
		Dim vlo_Resultado As EntDatosPaginacion
		...
		Try
			...
			vlo_Resultado = Me.ObtenerDatosPaginacionVista(vlo_Conexion, "V_TCC_CONDICION_LIBROLst", pvc_Condicion, pvc_Orden, pvn_TamanoPagina)
