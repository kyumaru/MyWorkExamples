<%@ Page Title="" Language="VB" MasterPageFile="~/MasterPage/Mp_Listado.master" AutoEventWireup="false" CodeFile="Lst_OT_VistoBuenoJefaturaSeccionMantenimiento.aspx.vb" Inherits="OrdenesDeTrabajo_Lst_OT_VistoBuenoJefaturaSeccionMantenimiento" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="cphFormulario" runat="Server">
    ...
    <header>
        <h2>Visto bueno de la jefatura Sección de Mantenimiento</h2>
    </header>
    ...
    <article data-grupo="Listado" class="tituloSeccion">
        Listado para Revisión de Ordenes de Trabajo
    </article>
        ...
        <asp:Repeater runat="server" ID="rpOrdenTrabajo">
            ...
            <ItemTemplate>
                <tr class="lineaDelListado">
                    ...
                    <td>
                        <article style="display: <%#IIf((CType(Eval(Utilerias.OrdenesDeTrabajo.Modelo.V_OTT_ORDEN_TRABAJOLST.ESTADO_ORDEN_TRABAJO), String) = Utilerias.OrdenesDeTrabajo.EstadoOrden.EVALUACION_PRELIMINAR_REVISION_JEFATURA), "block", "none")%>">
                            <a href="Frm_OT_VistoBuenoJefaturaSeccionMantenimiento.aspx?pvn_IdUbicacion=<%#Eval(Utilerias.OrdenesDeTrabajo.Modelo.V_OTT_ORDEN_TRABAJOLST.ID_UBICACION)%>&pvc_IdOrdenTrabajo=<%#Eval(Utilerias.OrdenesDeTrabajo.Modelo.V_OTT_ORDEN_TRABAJOLST.ID_ORDEN_TRABAJO)%>&pvn_Anno=<%#Eval(Utilerias.OrdenesDeTrabajo.Modelo.V_OTT_ORDEN_TRABAJOLST.ANNO)%>">
                    ...
                    <td>
                        <article style="display: <%#IIf((CType(Eval(Utilerias.OrdenesDeTrabajo.Modelo.V_OTT_ORDEN_TRABAJOLST.ESTADO_ORDEN_TRABAJO), String) = Utilerias.OrdenesDeTrabajo.EstadoOrden.EVALUADA), "block", "none")%>">
                            <a href="Frm_OT_ValidarOficio.aspx?pvn_IdUbicacion=<%#Eval(Utilerias.OrdenesDeTrabajo.Modelo.V_OTT_ORDEN_TRABAJOLST.ID_UBICACION)%>&pvc_IdOrdenTrabajo=<%#Eval(Utilerias.OrdenesDeTrabajo.Modelo.V_OTT_ORDEN_TRABAJOLST.ID_ORDEN_TRABAJO)%>&pvn_Anno=<%#Eval(Utilerias.OrdenesDeTrabajo.Modelo.V_OTT_ORDEN_TRABAJOLST.ANNO)%>">
                                <img title="Validar Oficio" alt="Validar Oficio"
                    ...
                    <td>
                        <article style="display: <%#IIf(Roles.IsUserInRole(Membership.GetUser.UserName, Utilerias.OrdenesDeTrabajo.RolesSistema.OT_JEFE_SECCION) Or Roles.IsUserInRole(Membership.GetUser.UserName, Utilerias.OrdenesDeTrabajo.RolesSistema.OT_DELEGADO_JEFE_SECCION), "block", "none")%>">
                            <asp:ImageButton runat="server" ID="imgValidarOficioPresupuesto" AlternateText="Validar oficio disponibilidad presupuestaria informe de valoración presupuestaria" ToolTip="Validar oficio disponibilidad presupuestaria informe de valoración presupuestaria"
                                Visible="<%#IIf(Eval(Utilerias.OrdenesDeTrabajo.Modelo.V_OTT_ORDEN_TRABAJOLST.ESTADO_ORDEN_TRABAJO).ToString = Utilerias.OrdenesDeTrabajo.EstadoOrden.PRESUPUESTO_APROBADO_SOLICITANTE, True, False)%>"
                                ...
                                onmouseover='<%#String.Format("this.src=""{0}""", AdministradorRecursos.ObtenerRutaImagen(AdministradorRecursos.TAMANO_IMAGEN.x16, AdministradorRecursos.COLOR_IMAGEN.COLOR, "Icono_Check.png"))%>'
                                onmouseout='<%#String.Format("this.src=""{0}""", AdministradorRecursos.ObtenerRutaImagen(AdministradorRecursos.TAMANO_IMAGEN.x16, AdministradorRecursos.COLOR_IMAGEN.GRIS, "Icono_Check.png"))%>'
                                onclick="imgValidarOficioPresupuesto_Click"/>
                        </article>
