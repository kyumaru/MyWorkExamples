...
Partial Class OrdenesDeTrabajo_Lst_OT_AutorizacionOrdenTrabajo
...
#Region "Métodos"
    ...
    Private Sub CargarLista(pvc_Condicion As String, pvc_Orden As String, pvn_NumeroDePagina As Integer)
        ...
        Try
            ...
            vlo_DsDatos = vlo_Ws_OT_OrdenesDeTrabajo.OTF_PRE_ORDEN_TRABAJO_ListarRegistrosLista(
                ConfigurationManager.AppSettings(ConstantesInternas.APP_KEY_USUARIO_APLICACION_WEB),
                ConfigurationManager.AppSettings(ConstantesInternas.APP_KEY_CLAVE_APLICACION_WEB),
                pvc_Condicion,
                pvc_Orden,
    ...
    Private Sub Buscar(pvc_Condicion As String, pvc_Orden As String)
        ...
        If String.IsNullOrWhiteSpace(pvc_Orden) Then
            pvc_Orden = ObtenerExpresionDeOrdenamiento(Modelo.V_OTF_PRE_ORDEN_TRABAJOLST.FECHA_HORA_SOLICITA)
        End If
        ...
        Try
            ...
            vlo_EntDatosPaginacion =
                vlo_Ws_OT_OrdenesDeTrabajo.OTF_PRE_ORDEN_TRABAJO_ObtenerDatosPaginacionVOtfPreOrdenTrabajolst(
                     ConfigurationManager.AppSettings(ConstantesInternas.APP_KEY_USUARIO_APLICACION_WEB),
                     ConfigurationManager.AppSettings(ConstantesInternas.APP_KEY_CLAVE_APLICACION_WEB),
                     pvc_Condicion,
                     pvc_Orden,
        ...
        Try
            If Not String.IsNullOrWhiteSpace(Me.txtIdentificacion.Text.Trim) Then
                If String.IsNullOrWhiteSpace(vlc_Condicion) Then
                    vlc_Condicion = String.Format("{0} LIKE '%{1}%'", Modelo.V_OTF_PRE_ORDEN_TRABAJOLST.ID_PERSONAL, Me.txtIdentificacion.Text.Trim)
                Else
                    vlc_Condicion = String.Format("{0} AND {1} LIKE '%{2}%'", vlc_Condicion, Modelo.V_OTF_PRE_ORDEN_TRABAJOLST.ID_PERSONAL, Me.txtIdentificacion.Text.Trim)
                End If
            End If

            If Not String.IsNullOrWhiteSpace(Me.ddlCategoriaServicio.SelectedValue) Then
                If String.IsNullOrWhiteSpace(vlc_Condicion) Then
                    vlc_Condicion = String.Format("{0} = {1}", Modelo.V_OTF_PRE_ORDEN_TRABAJOLST.ID_CATEGORIA_SERVICIO, Me.ddlCategoriaServicio.SelectedValue)
                Else
                    vlc_Condicion = String.Format("{0} AND {1} = {2}", vlc_Condicion, Modelo.V_OTF_PRE_ORDEN_TRABAJOLST.ID_CATEGORIA_SERVICIO, Me.ddlCategoriaServicio.SelectedValue)
                End If
            End If

            If String.IsNullOrWhiteSpace(vlc_Condicion) Then
                vlc_Condicion = String.Format("{0} LIKE '%{1}%'", Modelo.V_OTF_PRE_ORDEN_TRABAJOLST.ESTADO_ACTUAL_ORDEN_TRABAJO, EstadoOrden.PENDIENTE_REVISION_DIRECTOR)
            Else
                vlc_Condicion = String.Format("{0} AND {1} LIKE '%{2}%'", vlc_Condicion, Modelo.V_OTF_PRE_ORDEN_TRABAJOLST.ESTADO_ACTUAL_ORDEN_TRABAJO, EstadoOrden.PENDIENTE_REVISION_DIRECTOR)
            End If

            For Each vlo_Fila In Me.DsUnidades.Tables(0).Rows
                If String.IsNullOrWhiteSpace(vlc_SubCondicion) Then
                    vlc_SubCondicion = String.Format("{0} = {1}", Modelo.V_OTF_PRE_ORDEN_TRABAJOLST.COD_UNIDAD_SIRH, vlo_Fila("COD_UNIDAD_SIRH").ToString)
                Else
                    vlc_SubCondicion = String.Format("{0} OR {1} = {2}", vlc_SubCondicion, Modelo.V_OTF_PRE_ORDEN_TRABAJOLST.COD_UNIDAD_SIRH, vlo_Fila("COD_UNIDAD_SIRH").ToString)
                End If
            Next
