...
Partial Class OrdenesDeTrabajo_Almacen_Frm_OT_TrasladoDeMaterialABodega
...
#Region "Eventos"
            ...
            If vlo_almacen.Existe Then
                ...
                If vlo_EntOtfInventario.Existe Then
                    If vlo_EntOtfInventario.CantidadDisponible >= CInt(Me.WucDatosMaterial.RetornaCantidad) Then
                        ...
                        If (Me.WucDatosMaterial.RetornaCantidad <> String.Empty) AndAlso (CInt(Me.WucDatosMaterial.RetornaCantidad) > 0) Then
                            If Operacion = eOperacion.Agregar Then
                                If Not String.IsNullOrWhiteSpace(Me.txtCodigo.Text) Then

                                    If AgregarDsMateriales(Me.WucDatosMaterial.RetornaCantidad, 0, Me.WucDatosMaterial.RetornaDetalle, CType(vlc_monto(0), Integer), vlo_EntOtfInventario.CantidadDisponible, Me.WucDatosMaterial.RetornaDescripcion) Then
    ...
    ''' <creationDate>27/06/2016</creationDate>
    ''' <changeLog></changeLog>
    Protected Sub btnGuardarYFinalizar_Click(sender As Object, e As EventArgs) Handles btnEnviarAprobacion.Click
        If Page.IsValid Then
            Try
                If Me.CantidadMateriales > 0 Then
                    Estado = EstadoTraslado.PENDIENTE_APROBACION
                    If Operacion = eOperacion.Agregar Then
                        ...
                        End If
                    Else
                        If EnviarAprobacion(False) Then
    ...
    Protected Sub btnRegresar_Click(sender As Object, e As EventArgs) Handles btnRegresar.Click
        Try
            Me.Session.Add("pvn_IdBodega", Me.IdBodega)

        Catch ex As Exception
            Dim vlo_ControlDeErrores As New ControlDeErrores
            vlo_ControlDeErrores.RegistrarExcepcion(ex, String.Empty)
        End Try
        Response.Redirect("Lst_OT_TrasladoDeMaterialABodega.aspx", False)
    End Sub
...
#Region "Métodos"
    ...
    Private Sub LeerParametros()
        If Operacion = eOperacion.Agregar Then
            ...
            Me.Estado = EstadoTraslado.CREADA
        ...
        Else
            ...
            Me.IdBodega = WebUtils.LeerParametro(Of Integer)("pvn_IdBodega")
            Me.Estado = WebUtils.LeerParametro(Of String)("pvc_IdEstado")
    ...
    Private Sub InicializarFormulario()
        ...
        LeerParametros()
        Me.rblJornada.SelectedValue = Jornada.MANANA
        If Me.Estado.ToString.Equals(EstadoTraslado.CREADA) OrElse Me.Estado.ToString.Equals(EstadoTraslado.DEVUELTA) Then
            Me.btnGuardar.Visible = True
            Me.btnEnviarAprobacion.Visible = True
        Else
        ...
        Select Case Me.Operacion
            Case Is = eOperacion.Agregar
                CargarEstructuraDs()
                CargarEncargado()
                txtlblDevolucion.Visible = False
                txtObservacionesDevolucion.Visible = False
    ...
    Private Sub CargarListaMateriales(pvc_Orden As String, pvn_NumeroPaginacion As Integer)
        ...
        Try
            ...
            Me.DsMaterialesInsert = vlo_Ws_OT_Catalogos.OTT_LINEA_TRASLADO_ListarRegistrosLista(
                ConfigurationManager.AppSettings(ConstantesInternas.APP_KEY_USUARIO_APLICACION_WEB),
                ConfigurationManager.AppSettings(ConstantesInternas.APP_KEY_CLAVE_APLICACION_WEB),
                String.Format("{0} = '{1}' AND {2} = {3} AND {4} = '{5}' AND {6} = {7}", Modelo.V_OTT_LINEA_TRASLADOLST.ID_ALMACEN, Me.IdBodega, Modelo.V_OTT_LINEA_TRASLADOLST.ID_UBICACION, Me.IdUbicacion, Modelo.V_OTT_LINEA_TRASLADOLST.ESTADO, EstadoDetalle.PENDIENTE, Modelo.V_OTT_LINEA_TRASLADOLST.ID_SOLICITUD_TRASLADO, Me.IdSolicitudTraslado),
                pvc_Orden,
                True,
        ...
        Try
            ...
            If Me.SolicitudTraslado.Existe Then
                With Me.SolicitudTraslado
                    Me.txtFecha.Text = .FechaPropuestaSalida
                    Me.rblJornada.SelectedValue = .JornadaRetiro
                    If String.IsNullOrWhiteSpace(.Observaciones) Or .Observaciones.Equals("-") Then
                        Me.txtObservacione.Text = String.Empty
                    Else
                        Me.txtObservacione.Text = .Observaciones
                    End If
                    Me.lblNombreEncargado.Text = .Encargado
                End With
            End If

            If Not String.IsNullOrWhiteSpace(SolicitudTraslado.ObservacionesDevueltas) Then
                Me.txtObservacionesDevolucion.Text = SolicitudTraslado.ObservacionesDevueltas
                Me.txtObservacionesDevolucion.Visible = True
                Me.txtlblDevolucion.Visible = True
            Else
                Me.txtObservacionesDevolucion.Visible = False
                Me.txtlblDevolucion.Visible = False
            End If
    ...
    Private Sub CargarEncargado()
        ...
        Try
            ...
            vlo_EntOtmAlmacenBodega = vlo_Ws_OT_Catalogos.OTM_ALMACEN_BODEGA_ObtenerRegistro(
                ConfigurationManager.AppSettings(ConstantesInternas.APP_KEY_USUARIO_APLICACION_WEB),
                ConfigurationManager.AppSettings(ConstantesInternas.APP_KEY_CLAVE_APLICACION_WEB),
                String.Format("UPPER({0}) = {1} ", Modelo.OTM_ALMACEN_BODEGA.ID_ALMACEN_BODEGA, Me.IdBodega))

            If vlo_EntOtmAlmacenBodega.Existe Then
                vlo_EntOtmSectorTaller = vlo_Ws_OT_Catalogos.OTM_SECTOR_TALLER_ObtenerRegistroLista(
                    ConfigurationManager.AppSettings(ConstantesInternas.APP_KEY_USUARIO_APLICACION_WEB),
                    ConfigurationManager.AppSettings(ConstantesInternas.APP_KEY_CLAVE_APLICACION_WEB),
                    String.Format("UPPER({0}) = {1} ", Modelo.OTM_SECTOR_TALLER.ID_SECTOR_TALLER, vlo_EntOtmAlmacenBodega.IdSectorTaller))


                If Not String.IsNullOrWhiteSpace(vlo_EntOtmSectorTaller.NombreCoodinador) Then
                    Me.lblNombreEncargado.Text = vlo_EntOtmSectorTaller.NombreCoodinador
                    Me.lblEtiquetaEncargado.Visible = True
                    Me.lblNombreEncargado.Visible = True
                Else
                    Me.lblNombreEncargado.Text = String.Empty
                    Me.lblEtiquetaEncargado.Visible = False
                    Me.lblNombreEncargado.Visible = False
                End If
            Else
                Me.lblEtiquetaEncargado.Visible = False
                Me.lblNombreEncargado.Visible = False
    ...
    Private Sub CargarEstructuraDs()
        ...
        Try
            ...
            Me.DsMaterialesInsert.Tables(0).PrimaryKey = New DataColumn() {Me.DsMaterialesInsert.Tables(0).Columns(Modelo.V_OTT_LINEA_TRASLADOLST.ID_MATERIAL)}
            Me.CantidadMateriales = 0
...
#Region "Funciones"
    ...
    ''' <changeLog></changeLog>
    Private Function AgregarDsMateriales(pvn_cantidadRequerida As Integer, pvn_cantidadRetirada As Integer, pvc_detalle As String, pvn_costoPromedio As Integer, pvn_cantidadDisponible As Integer, pvc_descipcion As String) As Boolean
    ...
    ''' <changeLog></changeLog>
    Private Function EnviarAprobacion(ByVal pvb_EsAgregar As Boolean) As Boolean
        Dim vlo_Ws_OT_Catalogos As Ws_OT_Catalogos
        ...
        Try
            Return vlo_Ws_OT_Catalogos.OTT_SOLICITUD_TRASLADO_InsertarModificarRegistroConAsociados(
                ConfigurationManager.AppSettings(ConstantesInternas.APP_KEY_USUARIO_APLICACION_WEB),
                ConfigurationManager.AppSettings(ConstantesInternas.APP_KEY_CLAVE_APLICACION_WEB),
               vlo_EntOttSolicitudTraslado, DsMaterialesInsert, pvb_EsAgregar) > 0
    ...
    Private Function ConstruirRegistro() As Wsr_OT_Catalogos.EntOttSolicitudTraslado
        ...
        If Me.Operacion = eOperacion.Agregar Then
            ...
            With vlo_EntOttSolicitudTraslado
                ...
                .EstadoTraslado = Me.Estado
                .FechaPropuestaSalida = Me.txtFecha.Text
                .FechaRegistroSolicitud = Date.Now
                .JornadaRetiro = rblJornada.SelectedValue
                .Observaciones = Me.txtObservacione.Text
