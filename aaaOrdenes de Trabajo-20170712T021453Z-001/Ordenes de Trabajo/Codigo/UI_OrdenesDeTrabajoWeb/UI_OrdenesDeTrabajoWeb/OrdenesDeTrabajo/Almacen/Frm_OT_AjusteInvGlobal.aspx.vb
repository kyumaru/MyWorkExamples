...
Partial Class OrdenesDeTrabajo_Almacen_Frm_OT_AjusteInvGlobal
...
#Region "Eventos"
        ...
        If Not Page.IsPostBack Then
            Try
                ...
                If AutorizadoUbicacion.Existe Then
                    ...
                    Me.trPalabra.Visible = False

                    CargarLista(ObtenerCondicionBusqueda(), String.Format("{0} {1}", Modelo.V_OTF_INVENTARIOLST.ID_MATERIAL, Ordenamiento.ASCENDENTE))

                    If Me.IdAlmacen <> 0 Then
                        Me.ddlFiltroAlmacen.SelectedValue = Me.IdAlmacen
                        Me.ddlFiltroAlmacen.Enabled = False
    ...
    Protected Sub chkDetalle_CheckedChanged(sender As Object, e As EventArgs)
        Try
            ...
            vlo_Clave = vlo_CheckBox.Attributes("CommandArgument").ToString().Split("%")

            If vlo_CheckBox.Checked Then
                Me.DsMateriales.Tables(0).Rows.Find(New Object() {vlo_Clave(0), vlo_Clave(1), vlo_Clave(2)})(Modelo.V_OTF_INVENTARIOLST.SELECCIONADO) = "1"
            Else
                Me.DsMateriales.Tables(0).Rows.Find(New Object() {vlo_Clave(0), vlo_Clave(1), vlo_Clave(2)})(Modelo.V_OTF_INVENTARIOLST.SELECCIONADO) = "0"
            End If
        ...
        Try
            ...
            If ValidaGestion() Then
                ...
                If Me.Modo = 1 Then
                    ...
                    Me.Session.Add("pvn_IdAlmacen", Me.ddlFiltroAlmacen.SelectedValue)
                ...
                If Me.TipoAjuste = Utilerias.OrdenesDeTrabajo.TipoAjuste.EXISTENCA Then
                    Response.Redirect("Frm_OT_ConsultaAjusteInvExistencia.aspx", False)
                ElseIf Me.TipoAjuste = Utilerias.OrdenesDeTrabajo.TipoAjuste.GLOBALL Then
                    Response.Redirect("Frm_OT_ConsultaAjusteInvGlobal.aspx", False)
                End If
        ...
        Try
            Select Case ddlFiltrarPor.SelectedValue
                ...
                Case Is = FiltroPor.PARTIDA
                    ...
                    trPalabra.Visible = False
                Case Is = FiltroPor.PALABRA_CLAVE
                    trCategoria.Visible = False
                    trSubCategoria.Visible = False
                    trPartida.Visible = False
                    trPalabra.Visible = True
            End Select
            ...
            chkSeleccionarTodos.Checked = False
            txtFiltroPalabra.Text = String.Empty
...
#Region "Métodos"
    ...
    Private Sub CargarComboFiltrarPor()
        Try
            ...
            Me.ddlFiltrarPor.Items.Add(New ListItem(FiltroPor.PALABRA_CLAVE_STR, FiltroPor.PALABRA_CLAVE))
    ...
    Private Sub CargarComboAlmacenesBodegas()
        ...
        Try
            ...
            Me.ddlFiltroAlmacen.Items.Add(New ListItem(Constantes.FORMATO_DDL_SELECCIONE, String.Empty))
            ...
            For Each vlo_fila As Data.DataRow In vlo_dsDatos.Tables(0).Rows
                Me.ddlFiltroAlmacen.Items.Add(New ListItem(vlo_fila(Modelo.OTM_ALMACEN_BODEGA.DESCRIPCION), vlo_fila(Modelo.OTM_ALMACEN_BODEGA.ID_ALMACEN_BODEGA)))
            Next
        ...
        Try
            ...
            If Me.DsMateriales IsNot Nothing AndAlso Me.DsMateriales.Tables(0).Rows.Count > 0 Then
                With Me.rpDetalles
                    ...
                    .DataBind()
                    WebUtils.RegistrarScript(Me, "visibilidadPaneles", "ocultarAreaDeListado();") 'se oculta el listado ya que la seleccion de materiales  se realiza hasta que el usuario presiona buscar
                End With
            Else
                With Me.rpDetalles
                    .DataSource = Nothing
    ...
    Private Sub ProcesarSeleccion(pvb_SeleccionarTodos As Boolean)
        ...
        Try
            For Each item In Me.rpDetalles.Items
                ...
                If vlo_CheckBox.Checked Then
                    Me.DsMateriales.Tables(0).Rows.Find(New Object() {vlo_HiddenField.Value})(Modelo.V_OTF_INVENTARIOLST.SELECCIONADO) = "1"
                Else
                    Me.DsMateriales.Tables(0).Rows.Find(New Object() {vlo_HiddenField.Value})(Modelo.V_OTF_INVENTARIOLST.SELECCIONADO) = "0"
                End If
        ...
        Try
            For Each vlo_Row In Me.DsMateriales.Tables(0).Rows
                For Each vln_Registro In vlo_Clave
                    If vln_Registro = vlo_Row(Modelo.V_OTF_INVENTARIOLST.ID_MATERIAL) AndAlso Me.IdAlmacen = vlo_Row(Modelo.V_OTF_INVENTARIOLST.ID_ALMACEN_BODEGA) AndAlso Me.AutorizadoUbicacion.IdUbicacionAdministra = vlo_Row(Modelo.V_OTF_INVENTARIOLST.ID_UBICACION_ADMINISTRA) Then
                        vlo_Row(Modelo.V_OTF_INVENTARIOLST.SELECCIONADO) = "1"
                    End If
                Next
            Next
...
#Region "Funciones"
    ...
    Private Function ObtenerCondicionBusqueda() As String
        ...
        vlc_Condicion = String.Format("{0} = {1}", Modelo.V_OTF_INVENTARIOLST.ID_UBICACION_ADMINISTRA, Me.AutorizadoUbicacion.IdUbicacionAdministra)

        If Not String.IsNullOrWhiteSpace(Me.ddlFiltroAlmacen.SelectedValue) Then
            If String.IsNullOrWhiteSpace(vlc_Condicion) Then
                vlc_Condicion = String.Format("{0} = '{1}'", Modelo.V_OTF_INVENTARIOLST.ID_ALMACEN_BODEGA, Me.ddlFiltroAlmacen.SelectedValue)
            Else
                vlc_Condicion = String.Format("{0} AND {1} = '{2}'", vlc_Condicion, Modelo.V_OTF_INVENTARIOLST.ID_ALMACEN_BODEGA, Me.ddlFiltroAlmacen.SelectedValue)
            End If
        End If
            ...
            Select Case Me.ddlFiltrarPor.SelectedValue
                Case Is = FiltroPor.CATEGORIA
                    If Not String.IsNullOrWhiteSpace(Me.ddlFiltroCategoria.SelectedValue) Then
                        If String.IsNullOrWhiteSpace(vlc_Condicion) Then
                            vlc_Condicion = String.Format("{0} = '{1}'", Modelo.V_OTF_INVENTARIOLST.ID_CATEGORIA_MATERIAL, Me.ddlFiltroCategoria.SelectedValue)
                        Else
                            vlc_Condicion = String.Format("{0} AND {1} = '{2}'", vlc_Condicion, Modelo.V_OTF_INVENTARIOLST.ID_CATEGORIA_MATERIAL, Me.ddlFiltroCategoria.SelectedValue)
                        End If
                    End If

                    If Not String.IsNullOrWhiteSpace(Me.ddlFiltroSubCategoria.SelectedValue) Then
                        If String.IsNullOrWhiteSpace(vlc_Condicion) Then
                            vlc_Condicion = String.Format("{0} = '{1}'", Modelo.V_OTF_INVENTARIOLST.ID_SUBCATEGORIA_MATERIAL, Me.ddlFiltroSubCategoria.SelectedValue)
                        Else
                            vlc_Condicion = String.Format("{0} AND {1} = '{2}'", vlc_Condicion, Modelo.V_OTF_INVENTARIOLST.ID_SUBCATEGORIA_MATERIAL, Me.ddlFiltroSubCategoria.SelectedValue)
                        End If
                    End If
                ...
                Case Is = FiltroPor.PARTIDA
                    If Not String.IsNullOrWhiteSpace(Me.ddlFiltroPartida.SelectedValue) Then
                        If String.IsNullOrWhiteSpace(vlc_Condicion) Then
                            vlc_Condicion = String.Format("{0} = '{1}'", Modelo.V_OTF_INVENTARIOLST.PARTIDA_PRESUPUESTARIA, Me.ddlFiltroPartida.SelectedValue)
                        Else
                            vlc_Condicion = String.Format("{0} AND {1} = '{2}'", vlc_Condicion, Modelo.V_OTF_INVENTARIOLST.PARTIDA_PRESUPUESTARIA, Me.ddlFiltroPartida.SelectedValue)
                        End If
                    End If

                Case Is = FiltroPor.PALABRA_CLAVE
                    If Not String.IsNullOrWhiteSpace(Me.txtFiltroPalabra.Text) Then
                        If String.IsNullOrWhiteSpace(vlc_Condicion) Then
                            vlc_Condicion = String.Format("{0} = '{1}'", Modelo.V_OTF_INVENTARIOLST.DESC_MATERIAL, Me.txtFiltroPalabra.Text)
                        Else
                            vlc_Condicion = String.Format("{0} AND {1} = '{2}'", vlc_Condicion, Modelo.V_OTF_INVENTARIOLST.DESC_MATERIAL, Me.txtFiltroPalabra.Text)
                        End If
                    End If
            End Select
        End If
    ...
    Private Function ValidaGestion() As Boolean
        ...
        Try
            ...
            For Each vlo_FIla In Me.DsMateriales.Tables(0).Rows
                If vlo_FIla(Modelo.V_OTF_INVENTARIOLST.SELECCIONADO).ToString = "1" Then
