...
Partial Class OrdenesDeTrabajo_Almacen_Lst_OT_AtencionIncidentes
...
#Region "Eventos"
    ...
    Protected Sub Page_Load(sender As Object, e As EventArgs) Handles Me.Load
        If Not Page.IsPostBack Then
            Try
                If Roles.IsUserInRole(Membership.GetUser.UserName, RolesSistema.OT_ENCARGADO_INVENTARIO) Then
    ...
    ''' <summary>
    ''' Evento que se ejecuta cuando se da click sobre el boton de cada registro del listado
    ''' </summary>
    ''' <remarks></remarks>
    ''' <author>Carlos Gómez Ondoy</author>
    ''' <creationDate>08/08/2016</creationDate>
    ''' <changeLog></changeLog>
    Protected Sub ibAtenderIncidente_Click(sender As Object, e As ImageClickEventArgs)

        Try
            Me.Session.Add("pvn_IdIncidenteAlmacen", CType(sender, ImageButton).CommandArgument)

        Catch ex As Exception
            Dim vlo_ControlDeErrores As New ControlDeErrores
            vlo_ControlDeErrores.RegistrarExcepcion(ex, String.Empty)
        End Try
        Response.Redirect(String.Format("Frm_OT_AtencionIncidentes.aspx"), False)
    End Sub
...
#Region "Métodos"
    ...
    Private Sub CargarEstado()
        ...
        Me.ddlFiltroEstado.Items.Add(New ListItem("Pendiente", EstadoIncidente.PENDIENTE))
        Me.ddlFiltroEstado.Items.Add(New ListItem("Atendido", EstadoIncidente.ATENDIDO))
    End Sub
...
#Region "Funciones"
    ...
    Private Function ObtenerCondicionBusqueda() As String
        ...
        If Not String.IsNullOrWhiteSpace(Me.ddlFiltroEstado.SelectedValue) Then
            If String.IsNullOrWhiteSpace(vlc_Condicion) Then
                vlc_Condicion = String.Format("{0} = '{1}'", Modelo.V_OTF_INCIDENTE_ALMACENLST.ESTADO, Me.ddlFiltroEstado.SelectedValue)
            Else
                vlc_Condicion = String.Format("{0} AND {1} = '{2}'", vlc_Condicion, Modelo.V_OTF_INCIDENTE_ALMACENLST.ESTADO, Me.ddlFiltroEstado.SelectedValue)
            End If
        End If
        ...
        If String.IsNullOrWhiteSpace(vlc_Condicion) Then
            vlc_Condicion = String.Format("{0} = '{1}' OR {0} = '{2}'", Modelo.V_OTF_INCIDENTE_ALMACENLST.ESTADO, EstadoIncidente.PENDIENTE, EstadoIncidente.ATENDIDO)
        Else
            vlc_Condicion = String.Format("{0} AND ({1} = '{2}' OR {1} = '{3}')", vlc_Condicion, Modelo.V_OTF_INCIDENTE_ALMACENLST.ESTADO, EstadoIncidente.PENDIENTE, EstadoIncidente.ATENDIDO)
        End If
