...
Partial Class OrdenesDeTrabajo_Almacen_Lst_OT_AjusteInventario
...
#Region "Eventos"
    ...
    Protected Sub ibAgregar_Click(sender As Object, e As ImageClickEventArgs)
        Try
            Select Case Me.ddlTipoAjuste.SelectedValue
                'Suministros
                Case TipoAjuste.INDIVIDUAL
                    Me.Session.Add("pvn_Operacion", eOperacion.Agregar)
                    Response.Redirect(String.Format("Frm_OT_AjusteInvIndividual.aspx"), False)

                    'Unidad Especializada de Compras
                Case TipoAjuste.GLOBALL
                    Me.Session.Add("pvn_Modo", 1)
                    Me.Session.Add("pvc_TipoAjuste", TipoAjuste.GLOBALL)
                    Response.Redirect(String.Format("Frm_OT_AjusteInvGlobal.aspx"), False)

                    'Fondo de Trabajo
                Case TipoAjuste.EXISTENCA
                    Me.Session.Add("pvn_Modo", 1)
                    Me.Session.Add("pvc_TipoAjuste", TipoAjuste.EXISTENCA)
                    Response.Redirect(String.Format("Frm_OT_AjusteInvGlobal.aspx"), False)
                Case Else
                    MostrarAlertaError("Debe seleccionar el tipo de ajuste")
            End Select
        Catch ex As Exception
            Dim vlo_ControlDeErrores As New ControlDeErrores
            vlo_ControlDeErrores.RegistrarExcepcion(ex, String.Empty)
    ...
    Protected Sub lnkTrazabilidadGestion(sender As Object, e As CommandEventArgs)
        ...
        Me.Session.Add("pvc_PaginaRegreso", "Lst_OT_AjusteInventario.aspx")
    ...
    Protected Sub ibConsultarGestion_Click(sender As Object, e As ImageClickEventArgs)
        ...
        vlc_TipoAjuste = vlc_Llave(3)

        Me.Session.Add("pvc_TipoAjuste", vlc_TipoAjuste)

        If vlc_TipoAjuste = TipoAjuste.INDIVIDUAL Then
            Response.Redirect("Frm_OT_AjusteInvIndividual.aspx", False)
        ElseIf vlc_TipoAjuste = TipoAjuste.GLOBALL Then
            Me.Session.Add("pvn_Modo", 2)
            Response.Redirect("Frm_OT_ConsultaAjusteInvGlobal.aspx", False)
        ElseIf vlc_TipoAjuste = TipoAjuste.EXISTENCA Then
            Me.Session.Add("pvn_Modo", 2)
    ...
    Protected Sub ddlTipoAjuste_SelectedIndexChanged(sender As Object, e As EventArgs) Handles ddlTipoAjuste.SelectedIndexChanged
        Try
            If ddlTipoAjuste.SelectedValue = String.Empty Then
                Me.arNuevoRegistro.Visible = False
                Me.lnkNuevoRegistroFiltro.Visible = False
            Else
                Me.arNuevoRegistro.Visible = True
                Me.lnkNuevoRegistroFiltro.Visible = True
            End If
...
#Region "Métodos"
    ...
    Private Sub CargarEstado()
        ...
        Try
            ...
            vlo_DsDatos = vlo_Ws_OT_Catalogos.OTC_ESTADO_AJUSTE_ListarRegistrosLista(
            ...
            If vlo_DsDatos IsNot Nothing AndAlso vlo_DsDatos.Tables(0).Rows.Count > 0 Then
                ...
                With Me.ddlFiltroEstado
                    ...
                    .DataValueField = Modelo.OTC_ESTADO_AJUSTE.ESTADO_AJUSTE
                    .DataTextField = Modelo.OTC_ESTADO_AJUSTE.DESCRIPCION
                    .DataBind()
                End With
    ...
    Private Sub CargarComboTipoAjuste()
        Try

            Me.ddlTipoAjuste.Items.Clear()
            Me.ddlTipoAjuste.Items.Add(New ListItem(Constantes.FORMATO_DDL_TODOS, String.Empty))
            Me.ddlTipoAjuste.Items.Add(New ListItem(TipoAjuste.INDIVIDUAL_STR, TipoAjuste.INDIVIDUAL))
            Me.ddlTipoAjuste.Items.Add(New ListItem(TipoAjuste.GLOBAL_STR, TipoAjuste.GLOBALL))
            Me.ddlTipoAjuste.Items.Add(New ListItem(TipoAjuste.EXISTENCA_STR, TipoAjuste.EXISTENCA))

            ddlTipoAjuste.SelectedValue = String.Empty
    ...
    Private Sub Buscar(pvc_Condicion As String, pvc_Orden As String)
        ...
        Try
            ...
            vlo_EntDatosPaginacion = vlo_Ws_OT_OrdenesDeTrabajo.OTT_AJUSTE_INVENTARIO_ObtenerDatosPaginacionVOttAjusteInventariolst(
                         ConfigurationManager.AppSettings(ConstantesInternas.APP_KEY_USUARIO_APLICACION_WEB),
                         ConfigurationManager.AppSettings(ConstantesInternas.APP_KEY_CLAVE_APLICACION_WEB),
                         pvc_Condicion,
                         pvc_Orden,
            ...
            If vlo_EntDatosPaginacion.TotalRegistros > 0 Then
                ...
                Me.lblCantidadDeRegistrosSM.Text = String.Format("Cantidad de Ajustes {0}", vlo_EntDatosPaginacion.TotalRegistros)
...
#Region "Funciones"
    ...
    Private Function ObtenerCondicionBusqueda() As String
        ...
        If Not String.IsNullOrWhiteSpace(Me.ddlTipoAjuste.SelectedValue) Then
            If String.IsNullOrWhiteSpace(vlc_Condicion) Then
                vlc_Condicion = String.Format("{0} = '{1}'", Modelo.V_OTT_AJUSTE_INVENTARIOLST.TIPO_AJUSTE, Me.ddlTipoAjuste.SelectedValue)
            Else
                vlc_Condicion = String.Format("{0} AND {1} = '{2}'", vlc_Condicion, Modelo.V_OTT_AJUSTE_INVENTARIOLST.TIPO_AJUSTE, Me.ddlTipoAjuste.SelectedValue)
            End If
        End If
        ...
        If Not String.IsNullOrWhiteSpace(Me.ddlFiltroEstado.SelectedValue) Then
            If String.IsNullOrWhiteSpace(vlc_Condicion) Then
                vlc_Condicion = String.Format("{0} = '{1}'", Modelo.V_OTT_AJUSTE_INVENTARIOLST.ESTADO_AJUSTE, Me.ddlFiltroEstado.SelectedValue)
            Else
                vlc_Condicion = String.Format("{0} AND {1} = '{2}'", vlc_Condicion, Modelo.V_OTT_AJUSTE_INVENTARIOLST.ESTADO_AJUSTE, Me.ddlFiltroEstado.SelectedValue)
            End If
        End If
