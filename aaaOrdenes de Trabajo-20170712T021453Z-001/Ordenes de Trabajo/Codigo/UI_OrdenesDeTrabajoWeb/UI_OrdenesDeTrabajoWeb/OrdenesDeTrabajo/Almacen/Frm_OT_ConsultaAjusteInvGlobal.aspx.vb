...
Partial Class OrdenesDeTrabajo_Almacen_Frm_OT_ConsultaAjusteInvGlobal
...
#Region "Eventos"
    ...
    Protected Sub rpMateriales_ItemDataBound(sender As Object, e As RepeaterItemEventArgs) Handles rpMateriales.ItemDataBound
        ...
        Dim vlo_label As Label
        Dim vlo_ddlTipo As DropDownList
        ...
        If e.Item.ItemType = ListItemType.Item OrElse e.Item.ItemType = ListItemType.AlternatingItem Then
            ...
            vlo_label = e.Item.FindControl("lblAccion")

            If vlo_label IsNot Nothing Then

                vlo_ddlTipo = e.Item.FindControl("ddlTipo")
                If vlo_ddlTipo IsNot Nothing Then
                    vlo_ddlTipo.SelectedValue = vlo_label.Text
                End If
            End If
        End If
    End Sub
    ...
    Protected Sub ddlTipo_SelectedValueChanged(sender As Object, e As EventArgs)
        Dim ddlTipo As DropDownList = CType(sender, DropDownList)
        Dim vlc_Valor As String
        Dim vlo_Row As DataRow
        Try
            vlc_Valor = ddlTipo.SelectedValue

            vlo_Row = Me.DsMateriales.Tables(0).Rows.Find(New Object() {ddlTipo.Attributes("data-inf").ToString(), Me.IdAlmacen, Me.AutorizadoUbicacion.IdUbicacionAdministra})

            If Not String.IsNullOrWhiteSpace(vlc_Valor) Then
                vlo_Row("TIPO") = vlc_Valor
                If vlo_Row("CANTIDAD").ToString <> String.Empty Then
                    If vlc_Valor = TipoMovimiento.INCREMENTO Then
                        vlo_Row("POST_AJUSTE") = CType(vlo_Row(Modelo.V_OTF_INVENTARIOLST.CANTIDAD_DISPONIBLE), Double) + CType(vlo_Row("CANTIDAD"), Integer)
                    Else
                        vlo_Row("POST_AJUSTE") = CType(vlo_Row(Modelo.V_OTF_INVENTARIOLST.CANTIDAD_DISPONIBLE), Double) - CType(vlo_Row("CANTIDAD"), Integer)
                    End If
                End If
            ...
            Else
                ...
                vlo_Row("POST_AJUSTE") = String.Empty
            End If
        ...
        Try
            ...
            If Not String.IsNullOrWhiteSpace(vlc_Cantidad) Then
                If IsNumeric(vlc_Cantidad) Then
                    ...
                    If vlo_Row("TIPO").ToString <> String.Empty Then
                        If vlo_Row("TIPO").ToString = TipoMovimiento.INCREMENTO Then
                            vlo_Row("POST_AJUSTE") = CType(vlo_Row(Modelo.V_OTF_INVENTARIOLST.CANTIDAD_DISPONIBLE), Double) + CType(vlo_Row("CANTIDAD"), Integer)
                        Else
                            vlo_Row("POST_AJUSTE") = CType(vlo_Row(Modelo.V_OTF_INVENTARIOLST.CANTIDAD_DISPONIBLE), Double) - CType(vlo_Row("CANTIDAD"), Integer)
                        End If
...
#Region "Métodos"
    ...
    Private Sub CargarLista(pvc_HileraMateriales As String)
        ...
        Try
            ...
            Me.DsMateriales.Tables(0).Columns.Add("POST_AJUSTE", GetType(String))
                ...
                For Each vlo_FilaPantalla In Me.DsMateriales.Tables(0).Rows
                    If vlo_FilaPantalla(Modelo.V_OTF_INVENTARIOLST.ID_MATERIAL) = vlo_FilaDetalle(Modelo.OTT_DETALLE_AJUSTE.ID_MATERIAL) Then
                        If Not TypeOf vlo_FilaDetalle(Modelo.OTT_DETALLE_AJUSTE.DIRECCION_AJUSTE) Is DBNull Then
                            vlo_FilaPantalla("TIPO") = CType(vlo_FilaDetalle(Modelo.OTT_DETALLE_AJUSTE.DIRECCION_AJUSTE), String)
                        End If

                        If Not TypeOf vlo_FilaDetalle(Modelo.OTT_DETALLE_AJUSTE.CANTIDAD) Is DBNull Then
                            vlo_FilaPantalla("CANTIDAD") = CType(vlo_FilaDetalle(Modelo.OTT_DETALLE_AJUSTE.CANTIDAD), String)
                            If Not TypeOf vlo_FilaDetalle(Modelo.OTT_DETALLE_AJUSTE.DIRECCION_AJUSTE) Is DBNull Then
                                If vlo_FilaDetalle(Modelo.OTT_DETALLE_AJUSTE.DIRECCION_AJUSTE) = TipoMovimiento.INCREMENTO Then
                                    vlo_FilaPantalla("POST_AJUSTE") = CType(vlo_FilaPantalla(Modelo.V_OTF_INVENTARIOLST.CANTIDAD_DISPONIBLE), Double) + CType(vlo_FilaDetalle(Modelo.OTT_DETALLE_AJUSTE.CANTIDAD), Integer)
                                Else
                                    vlo_FilaPantalla("POST_AJUSTE") = CType(vlo_FilaPantalla(Modelo.V_OTF_INVENTARIOLST.CANTIDAD_DISPONIBLE), Double) - CType(vlo_FilaDetalle(Modelo.OTT_DETALLE_AJUSTE.CANTIDAD), Integer)
                                End If
                            End If
...
#Region "Funciones"
    ...
    Private Function CrearAjusteInventario(pvb_Finalizar As Boolean) As Boolean
        ...
        Try
            ...
            vlc_Resultado = vlo_Ws_OT_OrdenesDeTrabajo.OTT_AJUSTE_INVENTARIO_CrearAjusteGlobal(
                ConfigurationManager.AppSettings(ConstantesInternas.APP_KEY_USUARIO_APLICACION_WEB),
                ConfigurationManager.AppSettings(ConstantesInternas.APP_KEY_CLAVE_APLICACION_WEB),
    ...
    Private Function ModificarAjusteInventario(pvb_Finalizar As Boolean) As Boolean
        ...
        Try
            ...
            vlc_Resultado = vlo_Ws_OT_OrdenesDeTrabajo.OTT_AJUSTE_INVENTARIO_ModificarAjusteGlobal(
                ConfigurationManager.AppSettings(ConstantesInternas.APP_KEY_USUARIO_APLICACION_WEB),
                ConfigurationManager.AppSettings(ConstantesInternas.APP_KEY_CLAVE_APLICACION_WEB),
