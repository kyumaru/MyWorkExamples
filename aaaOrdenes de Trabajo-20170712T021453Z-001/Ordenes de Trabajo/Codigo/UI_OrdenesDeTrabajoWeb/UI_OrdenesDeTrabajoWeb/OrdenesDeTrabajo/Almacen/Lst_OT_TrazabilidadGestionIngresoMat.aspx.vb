...
Partial Class OrdenesDeTrabajo_Almacen_Lst_OT_TrazabilidadGestionIngresoMat
...
#Region "Eventos"
    ...
    Protected Sub btnRegresar_Click(sender As Object, e As EventArgs) Handles btnRegresar.Click
        ...
        Response.Redirect(Me.txtPaginaRegreso.Text, False)
    End Sub
...
#Region "Metodos"
    Private Sub InicializarFormulario()
        ...
        Try
            ...
            Me.Consecutivo = CType(Session("pvn_Consecutivo"), Integer)
            Me.txtPaginaRegreso.Text = CType(Session("pvc_PaginaRegreso"), String)

            Me.txtObservaciones.Enabled = False


            vlc_Condicion = Me.Condicion = String.Format("{0} = {1} AND {2} = {3} AND {4} = {5}", Modelo.V_OTT_GESTION_COMPRALST.ID_UBICACION, Me.IdUbicacion, Modelo.V_OTT_GESTION_COMPRALST.ID_VIA_COMPRA_CONTRATO, Me.IdViaCompraContrato, Modelo.V_OTT_GESTION_COMPRALST.ANNO, Me.Anno, Modelo.V_OTT_GESTION_COMPRALST.NUMERO_GESTION, Me.NumeroGestion)
            ...
            Me.lblTitulo.Text = "Seguimiento a Gestiones de Ingreso de Material"
            ...
            Me.lblEstado.Text = vlo_DsDatos.Tables(0).Rows(0).Item(Modelo.V_OTT_GESTION_COMPRALST.DESC_ESTADO)
            ...
            Me.Condicion = String.Format("{0} = {1} AND {2} = {3} AND {4} = {5} AND {6} = {7}", Modelo.V_OTL_TRAZABIL_GESTION_INGRLST.ID_UBICACION, Me.IdUbicacion, Modelo.V_OTL_TRAZABIL_GESTION_INGRLST.ID_VIA_COMPRA_CONTRATO, Me.IdViaCompraContrato, Modelo.V_OTL_TRAZABIL_GESTION_INGRLST.ANNO, Me.Anno, Modelo.V_OTL_TRAZABIL_GESTION_INGRLST.NUMERO_GESTION, Me.NumeroGestion, Modelo.V_OTL_TRAZABIL_GESTION_INGRLST.CONSECUTIVO, Me.Consecutivo)

            BuscarTramites(Me.Condicion, String.Format("{0} {1}", Modelo.V_OTL_TRAZABIL_GESTION_INGRLST.TIME_STAMP, Ordenamiento.DESCENDENTE))
    ...
    Private Sub CargarListaTramites(pvc_Condicion As String, pvc_Orden As String, pvn_NumeroDePagina As Integer)
        ...
        Try
            vlo_DsDatos = vlo_Ws_OT_OrdenesDeTrabajo.OTL_TRAZABIL_GESTION_INGR_ListarRegistrosLista(
                ConfigurationManager.AppSettings(ConstantesInternas.APP_KEY_USUARIO_APLICACION_WEB),
                ConfigurationManager.AppSettings(ConstantesInternas.APP_KEY_CLAVE_APLICACION_WEB),
                pvc_Condicion,
                pvc_Orden,
    ...
    Private Sub BuscarTramites(pvc_Condicion As String, pvc_Orden As String)
        ...
        Try
            vlo_EntDatosPaginacion = vlo_Ws_OT_OrdenesDeTrabajo.OTL_TRAZABIL_GESTION_INGR_ObtenerDatosPaginacionVOtlTrazabilGestionIngrlst(
                ConfigurationManager.AppSettings(ConstantesInternas.APP_KEY_USUARIO_APLICACION_WEB),
                ConfigurationManager.AppSettings(ConstantesInternas.APP_KEY_CLAVE_APLICACION_WEB),
                pvc_Condicion,
                pvc_Orden,
