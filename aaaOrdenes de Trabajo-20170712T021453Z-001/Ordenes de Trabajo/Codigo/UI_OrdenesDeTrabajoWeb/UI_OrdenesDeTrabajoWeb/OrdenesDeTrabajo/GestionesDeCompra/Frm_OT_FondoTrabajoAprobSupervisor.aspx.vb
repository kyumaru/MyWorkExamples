...
Partial Class OrdenesDeTrabajo_GestionesDeCompra_Frm_OT_FondoTrabajoAprobSupervisor
    ...
    Protected Sub Page_Load(sender As Object, e As EventArgs) Handles Me.Load
        If Not IsPostBack Then
            Try
                ...
                Me.UltimaCondicionBusquedaArchivo = String.Format("{0} = {1} AND {2} = {3} AND {4} = {5} AND {6} = {7}", Modelo.V_OTT_ADJUNTO_COTIZACIONLST.ID_UBICACION, Me.IdUbicacion, Modelo.V_OTT_ADJUNTO_COTIZACIONLST.ID_VIA_COMPRA_CONTRATO, Me.IdViaCompraContrato, Modelo.V_OTT_ADJUNTO_COTIZACIONLST.ANNO, Me.Anno, Modelo.V_OTT_ADJUNTO_COTIZACIONLST.NUMERO_GESTION, Me.NumeroGestion)
                BuscarArchivo(Me.UltimaCondicionBusquedaArchivo, String.Empty)

                'Se carga el combo de proveedores
                CargarComboProveedores()

                Me.trAdjudicado.Visible = False
                Me.trObservaciones.Visible = False
                Me.txtObservaciones.MaxLength = Modelo.OTT_GESTION_COMPRA.OBSERVACIONES_BD_TAMANO
...
#Region "Métodos"
    ...
    Private Sub CargarComboProveedores()
        ...
        Try
            Me.ddlAdjudicado.Items.Clear()
            Me.ddlAdjudicado.Items.Add(New ListItem(Constantes.FORMATO_DDL_SELECCIONE, String.Empty))

            vlo_DsDatos = vlo_Ws_OT_OrdenesDeTrabajo.OTT_OFERTA_PROVEEDOR_ListarRegistrosLista(
                ConfigurationManager.AppSettings(ConstantesInternas.APP_KEY_USUARIO_APLICACION_WEB),
                ConfigurationManager.AppSettings(ConstantesInternas.APP_KEY_CLAVE_APLICACION_WEB),
                String.Format("{0} = {1} AND {2} = {3} AND {4} = {5} AND {6} = {7} AND {8} = 1", Modelo.V_OTT_OFERTA_PROVEEDORLST.ID_UBICACION, Me.IdUbicacion, Modelo.V_OTT_OFERTA_PROVEEDORLST.ID_VIA_COMPRA_CONTRATO, Me.IdViaCompraContrato, Modelo.V_OTT_OFERTA_PROVEEDORLST.ANNO, Me.Anno, Modelo.V_OTT_OFERTA_PROVEEDORLST.NUMERO_GESTION, Me.NumeroGestion, Modelo.V_OTT_OFERTA_PROVEEDORLST.TIENE_ADJUNTO),
                String.Empty,
                False,
                0,
                0)

            'asigna los datos obtenidos al drop down list
            If vlo_DsDatos IsNot Nothing AndAlso vlo_DsDatos.Tables(0).Rows.Count > 0 Then
                Me.ddlAdjudicado.DataSource = vlo_DsDatos
                Me.ddlAdjudicado.DataTextField = Modelo.V_OTT_OFERTA_PROVEEDORLST.NOMBRE_PROVEEDOR
                Me.ddlAdjudicado.DataValueField = Modelo.V_OTT_OFERTA_PROVEEDORLST.IDENTIFICACION
                Me.ddlAdjudicado.DataBind()
            End If
    ...
    Private Sub GuardarRevision()
        ...
        Try
            ...
            If vlo_EntOttGestionCompra.Existe Then
                vlo_EntOttGestionCompra.Estado = IIf(Me.rdbAprobado.Checked, Utilerias.OrdenesDeTrabajo.EstadoGestionCompra.APROBACION_DE_PRESUPUESTO, Utilerias.OrdenesDeTrabajo.EstadoGestionCompra.REGISTRO_DE_COTIZACIONES)
            End If
            ...
            vln_Resultado = vlo_Wsr_OT_OrdenesDeTrabajo.OTT_GESTION_COMPRA_FondoTrabajoAprobSupervisor(System.Configuration.ConfigurationManager.AppSettings(ConstantesInternas.APP_KEY_USUARIO_APLICACION_WEB).ToString, System.Configuration.ConfigurationManager.AppSettings(ConstantesInternas.APP_KEY_CLAVE_APLICACION_WEB).ToString, vlo_EntOttGestionCompra, vlo_EntOtlTrazabilGestionComp, Me.ddlAdjudicado.SelectedValue)
