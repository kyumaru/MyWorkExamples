...
Partial Class OrdenesDeTrabajo_GestionesDeCompra_Lst_OT_GestiónCompraRapida
...
#Region "Propiedades"
    ...
    ''' <creationDate>19/08/2016</creationDate>
    ''' <changeLog></changeLog>
...
#Region "Eventos"
    ...
    Protected Sub Page_Load(sender As Object, e As EventArgs) Handles Me.Load
        If Not Page.IsPostBack Then
            Try
                ...
                If Roles.IsUserInRole(Membership.GetUser.UserName, RolesSistema.OT_ENCARGADO_COMPRA_RAPIDA) Then
    ...
    ''' <creationDate>19/08/2016</creationDate>
    ''' <changeLog></changeLog>
    Protected Sub lnkRpCompraRapida_Command(sender As Object, e As CommandEventArgs)
        Try
            CargarLista(ObtenerCondicionBusqueda, ObtenerExpresionDeOrdenamiento(e.CommandName), Me.pnRpCompraRapida.PaginaActualLista)
        Catch ex As Exception
            Dim vlo_ControlDeErrores As New ControlDeErrores
            vlo_ControlDeErrores.RegistrarExcepcion(ex, String.Empty)
        End Try
    ...
    ''' <creationDate>19/08/2016</creationDate>
    ''' <changeLog></changeLog>
    Protected Sub pnRpCompraRapida_CambioDePagina(pvn_PaginaSeleccionada As Integer) Handles pnRpCompraRapida.CambioDePagina
        Try
    ...
    Private Sub Buscar(pvc_Condicion As String, pvc_Orden As String)
        ...
        Try
            ...
            If vlo_EntDatosPaginacion.TotalRegistros > 0 Then
                Me.pnRpCompraRapida.TotalPaginasLista = vlo_EntDatosPaginacion.TotalPaginas
...
#Region "Funciones"
    ...
    Private Function ObtenerCondicionBusqueda() As String
        ...
        If Not String.IsNullOrWhiteSpace(Me.txtFiltroFechaDesde.Text) And Not String.IsNullOrWhiteSpace(Me.txtFiltroFechaHasta.Text) Then
            If String.IsNullOrWhiteSpace(vlc_Condicion) Then
                vlc_Condicion = String.Format("{0} BETWEEN TO_DATE('{1}', 'dd/mm/yyyy') AND TO_DATE('{2}', 'dd/mm/yyyy')", Modelo.V_OTT_GESTION_COMPRALST.FECHA_REGISTRO_SOLICITUD, Me.txtFiltroFechaDesde.Text, Me.txtFiltroFechaHasta.Text)
            Else
                vlc_Condicion = String.Format("{0} AND {1} BETWEEN TO_DATE('{2}', 'dd/mm/yyyy') AND TO_DATE('{3}', 'dd/mm/yyyy')", vlc_Condicion, Modelo.V_OTT_GESTION_COMPRALST.FECHA_REGISTRO_SOLICITUD, Me.txtFiltroFechaDesde.Text, Me.txtFiltroFechaHasta.Text)
            End If
        End If

         If String.IsNullOrWhiteSpace(vlc_Condicion) Then
            vlc_Condicion = String.Format("{0} = '{1}'", Modelo.V_OTT_GESTION_COMPRALST.ESTADO, EstadoGestionCompra.CREADA)
        Else
            vlc_Condicion = String.Format("{0} AND {1} = '{2}'", vlc_Condicion, Modelo.V_OTT_GESTION_COMPRALST.ESTADO, EstadoGestionCompra.CREADA)
        End If
        ...
        If String.IsNullOrWhiteSpace(vlc_Condicion) Then
            vlc_Condicion = String.Format("{0} = {1}", Modelo.V_OTT_GESTION_COMPRALST.ID_VIA_COMPRA_CONTRATO, Me.ParametroViaCompraContrato.Valor)
        Else
            vlc_Condicion = String.Format("{0} AND {1} = {2}", vlc_Condicion, Modelo.V_OTT_GESTION_COMPRALST.ID_VIA_COMPRA_CONTRATO, Me.ParametroViaCompraContrato.Valor)
        End If
