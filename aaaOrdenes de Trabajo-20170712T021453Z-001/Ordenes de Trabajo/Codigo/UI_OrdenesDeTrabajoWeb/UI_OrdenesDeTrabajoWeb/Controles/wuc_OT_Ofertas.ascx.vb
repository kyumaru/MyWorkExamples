...
Partial Class Controles_wuc_OT_Ofertas
...
#Region "Eventos"
    ...
    'Protected Sub chkContratacionDesierta_CheckedChanged(sender As Object, e As EventArgs)
    '    Try
    '        If Me.chkContratacionDesierta.Checked Then
    '            Me.txtResumenOferta.Visible = False
    '            Me.btnAgregarDocumento.Visible = False
    '            Me.fuDocumento.Visible = False
    '            Me.arAgregarArchivo.Visible = False
    '        Else
    '            Me.txtResumenOferta.Visible = True
    '            Me.btnAgregarDocumento.Visible = True
    '            Me.fuDocumento.Visible = True
    '            Me.arAgregarArchivo.Visible = True
    '        End If
    ...
    ''' <creationDate>27/04/2016</creationDate>
    ''' <changeLog></changeLog>
    Protected Sub rpActividadesEjecucion_ItemDataBound(sender As Object, e As RepeaterItemEventArgs) Handles rpAdjuntos.ItemDataBound
    ...
    ''' <creationDate>27/04/2016</creationDate>
    ''' <changeLog></changeLog>
    Protected Sub ibBorrarAdjuntoOfertas_Click(sender As Object, e As ImageClickEventArgs)
        ...
        Try
            ...
            If Borrar(vln_IdAdjunto) Then
                ...
                If DsArchivos.Tables(0).Rows.Count = 1 Then
                    DsArchivos = New DataSet
                End If
                CargarDatos()
                
            End If
            If Me.Cargo = Utilerias.OrdenesDeTrabajo.Cargo.PROFESIONAL Then
    ...
    Protected Sub btnAgregarArchivo_Click(sender As Object, e As EventArgs) Handles btnAgregarDocumento.Click
        Try
            ...
            Me.txtResumenOferta.Text = String.Empty
            If Me.Cargo = Utilerias.OrdenesDeTrabajo.Cargo.PROFESIONAL Then
                WebUtils.RegistrarScript(Me, "activarRecomendacion", "activarRecomendacion();")
            Else
                WebUtils.RegistrarScript(Me, "activarOfertas", "activarOfertas();")
    ...
    Protected Sub btnCerrarEtapa_Click(sender As Object, e As EventArgs)
        Try
            If Editable = Utilerias.OrdenesDeTrabajo.Version.EDITABLE AndAlso ValidaTramitados() Then
                ...
                End If
            Else
                MostrarAlertaError("No se puede cerrar esta etapa hasta que todas las revisiones del profesional esten tramitadas.")
            End If
            WebUtils.RegistrarScript(Me, "activarOfertas", "activarOfertas();")
        Catch ex As Exception
            If TypeOf (ex) Is System.Web.Services.Protocols.SoapException AndAlso
    ...
    Protected Sub chkTramitado_CheckedChanged(sender As Object, e As EventArgs)
        ...
        Try
            ...
            If Modificar(vlc_IdAdjunto) Then
...
#Region "Metodos"
    ...
    Private Sub InicializarFormulario()
        ...
        If Me.EtapaActual = EtapaContratacion.OFERTAS AndAlso Editable = Utilerias.OrdenesDeTrabajo.Version.EDITABLE Then
            If Me.Cargo = Utilerias.OrdenesDeTrabajo.Cargo.ENCARGADO Then
                btnCerrarEtapa.Visible = True
            Else
                chkContratacionDesierta.Visible = False
                btnCerrarEtapa.Visible = False
            End If
            btnAgregarDocumento.Enabled = True
            chkContratacionDesierta.Visible = True
        Else
        ...
        Try
            ...
            Me.DsArchivos = vlo_Ws_OT_OrdenesDeTrabajo.OTT_DOCUMENTO_CONTRATACION_ListarRegistrosLista(
                ...
                String.Format("{0} = '{1}' AND {2} = {3} AND {4} = {5} AND {6} = {7}",
                              ...
                              EtapaContratacion.OFERTAS, Modelo.OTT_DOCUMENTO_CONTRATACION.VERSION,
            ...
            If Me.DsArchivos IsNot Nothing AndAlso Me.DsArchivos.Tables(0).Rows.Count > 0 Then
                Me.chkContratacionDesierta.Visible = False
            ...
            Else
                ...
                End With
                    ...
                    Me.chkContratacionDesierta.Visible = True
            End If
    ...
    Private Sub CargarTiposDocumento()
        ...
        Try
            ...
            If vlo_EntOtmTipoDocumento.Existe Then
                ...
                imgExtensionesOfertas.Attributes.Add("title", String.Format("Extensiones permitidas:{0}", ExtensionesPermitidas.ToLower))
    ...
    Private Sub AgregarArchivo()
        ...
        Try
            ...
            vlo_NuevoArchivo.Descripcion = Me.txtResumenOferta.Text
            ...
            If vln_Resultado > 0 Then
                ...
                vlo_EntOttDocumentoContratacion.IdEtapaContratacion = EtapaContratacion.OFERTAS
    ...
    ''' <creationDate>27/04/2016</creationDate>
    ''' <changeLog></changeLog>
    Private Function Modificar(pvc_IdAdjunto As String) As Boolean
        Dim vln_Resultado As Integer
        Dim vlo_Ws_OT_OrdenesDeTrabajo As Wsr_OT_OrdenesDeTrabajo.Ws_OT_OrdenesDeTrabajo
        Dim vlo_EntOttDocumentoContratacion As Wsr_OT_OrdenesDeTrabajo.EntOttDocumentoContratacion
...
#Region "Funciones"
    ...
    ''' <creationDate>27/04/2016</creationDate>
    ''' <changeLog></changeLog>
    Private Function CerrarOfertas() As Boolean
        Dim vlo_Ws_OT_OrdenesDeTrabajo As Wsr_OT_OrdenesDeTrabajo.Ws_OT_OrdenesDeTrabajo
        ...
        Try
            If Me.chkContratacionDesierta.Checked Then
                Return vlo_Ws_OT_OrdenesDeTrabajo.OTT_CONTRATACION_CerrarEtapaOfertas(
                    ConfigurationManager.AppSettings(ConstantesInternas.APP_KEY_USUARIO_APLICACION_WEB),
                    ConfigurationManager.AppSettings(ConstantesInternas.APP_KEY_CLAVE_APLICACION_WEB),
                    Me.IdOrdenTrabajo, IdUbicacion, Me.Usuario.NumEmpleado, Me.EtapaActual, Me.chkContratacionDesierta.Checked, Version) > 0
            End If
            If Me.DsArchivos.Tables.Count > 0 AndAlso Me.DsArchivos.Tables(0).Rows.Count > 0 Then
                Return vlo_Ws_OT_OrdenesDeTrabajo.OTT_CONTRATACION_CerrarEtapaOfertas(
                    ...
                    Me.IdOrdenTrabajo, IdUbicacion, Me.Usuario.NumEmpleado, Me.EtapaActual, Me.chkContratacionDesierta.Checked, Version) > 0
            Else
                MostrarAlertaError("Debe adjuntar al menos un documento para cerrar la etapa de ofertas.")
            End If
    ...
    Private Function ValidaTramitados() As Boolean
        ...
        Try
            If DsArchivos.Tables.Count > 0 Then
                ...
                vlo_resultado = vlo_filas.Length <= 0
            Else
                vlo_resultado = False
            End If
