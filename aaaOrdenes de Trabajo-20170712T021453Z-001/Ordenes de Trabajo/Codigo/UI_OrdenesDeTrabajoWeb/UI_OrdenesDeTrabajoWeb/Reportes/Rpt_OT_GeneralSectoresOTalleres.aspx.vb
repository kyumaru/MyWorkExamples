...
Partial Class Reportes_Rpt_OT_GeneralSectoresOTalleres
...
#Region "Eventos"
    ...
    Protected Sub btnGenerar_Click(sender As Object, e As EventArgs) Handles btnGenerar.Click
        Try
            ...
            Response.Redirect(String.Format("RPT_OT_ReportePDF.aspx?pvc_Condicion={0}&pvc_Orden={1}&pvc_NombreReporte={2}&pvc_CondicionUsuario={3}",
                                            ObtenerCondicionBusqueda(), ObtenerOrden(), Reportes.RPT_OT_REPORTE_GENERAL_SECTORES_TALLERES, CondicionUsuario))
        Catch ex As Exception
        ...
        Try
            ...
            ElseIf Roles.IsUserInRole(Membership.GetUser.UserName, Utilerias.OrdenesDeTrabajo.RolesSistema.OT_COORDINADOR_MANTENIMIENTO) Or Roles.IsUserInRole(Membership.GetUser.UserName, Utilerias.OrdenesDeTrabajo.RolesSistema.OT_COORDINADOR_DISENIO) Then
                ...
                vlo_DsDatos = vlo_Ws_OT_Catalogos.OTM_SECTOR_TALLER_ListarRegistros(
                    ConfigurationManager.AppSettings(ConstantesInternas.APP_KEY_USUARIO_APLICACION_WEB),
                    ConfigurationManager.AppSettings(ConstantesInternas.APP_KEY_CLAVE_APLICACION_WEB),
                    String.Format("{0}={1}", Modelo.OTM_SECTOR_TALLER.NUM_EMPLEADO_COORDINADOR, Me.Usuario.NumEmpleado), String.Empty, False, 1, 0)
            End If
