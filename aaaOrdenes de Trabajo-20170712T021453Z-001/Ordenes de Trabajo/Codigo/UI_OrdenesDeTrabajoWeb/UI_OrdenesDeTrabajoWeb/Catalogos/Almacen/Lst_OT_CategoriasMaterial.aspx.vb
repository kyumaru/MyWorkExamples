...
Partial Class Catalogos_Lst_OT_CategoriasMaterial
...
#Region "Eventos"
    ...
    ''' <creationDate>20/05/2016</creationDate>
    ''' <changeLog></changeLog>
    Protected Sub lnkRpCategoria_Command(sender As Object, e As CommandEventArgs)
        Try
            CargarLista(ObtenerCondicionBusqueda, ObtenerExpresionDeOrdenamiento(e.CommandName), Me.pnRpCategoria.PaginaActualLista)
        Catch ex As Exception
            Dim vlo_ControlDeErrores As New ControlDeErrores
            vlo_ControlDeErrores.RegistrarExcepcion(ex, String.Empty)
        End Try
    ...
    ''' <creationDate>20/05/2016</creationDate>
    ''' <changeLog></changeLog>
    Protected Sub pnRpCategoria_CambioDePagina(pvn_PaginaSeleccionada As Integer) Handles pnRpCategoria.CambioDePagina
        Try
    ...
    ''' <creationDate>20/05/2016</creationDate>
    ''' <changeLog></changeLog>
    Protected Sub rpCategoria_ItemDataBound(sender As Object, e As RepeaterItemEventArgs) Handles rpCategoria.ItemDataBound
...
#Region "Métodos"
    ...
    Private Sub Buscar(pvc_Condicion As String, pvc_Orden As String)
        ...
        If String.IsNullOrWhiteSpace(pvc_Orden) Then
            pvc_Orden = ObtenerExpresionDeOrdenamiento(Modelo.V_OTM_CATEGORIA_MATERIALLST.DESCRIPCION)
        End If
        ...
        Try
            ...
            vlo_EntDatosPaginacion =
                vlo_Ws_OT_Catalogos.OTM_CATEGORIA_MATERIAL_ObtenerDatosPaginacionVOtmCategoriaMateriallst(
                     ConfigurationManager.AppSettings(ConstantesInternas.APP_KEY_USUARIO_APLICACION_WEB),
                     ConfigurationManager.AppSettings(ConstantesInternas.APP_KEY_CLAVE_APLICACION_WEB),
                     pvc_Condicion,
                     pvc_Orden,
            ...
            If vlo_EntDatosPaginacion.TotalRegistros > 0 Then
                Me.pnRpCategoria.TotalPaginasLista = vlo_EntDatosPaginacion.TotalPaginas
                CargarLista(pvc_Condicion, pvc_Orden, 1)
                Me.pnRpCategoria.Dibujar()
                Me.lblCantidadDeRegistros.Visible = True
                Me.lblCantidadDeRegistros.Text = String.Format("Cantidad de Categorías {0}", vlo_EntDatosPaginacion.TotalRegistros)
            Else
...
#Region "Funciones"
    ...
    ''' </summary>
    ''' <param name="pvn_IdCategoriaMaterial"></param>
    ''' <returns>Si retorna un número mayor a 0 quiere decir que la operacion se realizo con éxito</returns>
    ''' <remarks></remarks>
    ''' <author>Carlos Gómez Ondoy</author>
    ''' <creationDate>20/05/2016</creationDate>
    ''' <changeLog></changeLog>
    Private Function Borrar(pvn_IdCategoriaMaterial As Integer) As Boolean
        Dim vlo_Ws_OT_Catalogos As Wsr_OT_Catalogos.Ws_OT_Catalogos
        ...
        vlo_EntOtmcategoriaMaterial.IdCategoriaMaterial = pvn_IdCategoriaMaterial

        Try
            Return vlo_Ws_OT_Catalogos.OTM_CATEGORIA_MATERIAL_BorrarRegistroConAsociados(
                ConfigurationManager.AppSettings(ConstantesInternas.APP_KEY_USUARIO_APLICACION_WEB),
                ConfigurationManager.AppSettings(ConstantesInternas.APP_KEY_CLAVE_APLICACION_WEB),
                vlo_EntOtmcategoriaMaterial) > 0
        Catch ex As Exception
            Throw
        Finally
            If vlo_Ws_OT_Catalogos IsNot Nothing Then
    ...
    Private Function ObtenerCondicionBusqueda() As String
        ...
        If Not String.IsNullOrWhiteSpace(Me.txtDescripcion.Text.Trim) Then
            If String.IsNullOrWhiteSpace(vlc_Condicion) Then
                vlc_Condicion = String.Format("UPPER({0}) LIKE '%{1}%'", Modelo.V_OTM_CATEGORIA_MATERIALLST.DESCRIPCION, Me.txtDescripcion.Text.Trim.ToUpper)
            Else
                vlc_Condicion = String.Format("{0} AND UPPER({1}) LIKE '%{2}%'", vlc_Condicion, Modelo.V_OTM_CATEGORIA_MATERIALLST.DESCRIPCION, Me.txtDescripcion.Text.Trim.ToUpper)
            End If
        End If

        If Not String.IsNullOrWhiteSpace(Me.ddlFiltroEstado.SelectedValue) Then
            If String.IsNullOrWhiteSpace(vlc_Condicion) Then
                vlc_Condicion = String.Format("{0} = '{1}'", Modelo.V_OTM_CATEGORIA_MATERIALLST.ESTADO, Me.ddlFiltroEstado.SelectedValue)
            Else
                vlc_Condicion = String.Format("{0} AND {1} = '{2}'", vlc_Condicion, Modelo.V_OTM_CATEGORIA_MATERIALLST.ESTADO, Me.ddlFiltroEstado.SelectedValue)
            End If
        End If

        If String.IsNullOrWhiteSpace(vlc_Condicion) Then
            vlc_Condicion = String.Format("{0} = {1}", Modelo.V_OTM_CATEGORIA_MATERIALLST.ID_UBICACION_ADMINISTRA, Me.AutorizadoUbicacion.IdUbicacionAdministra)
        Else
            vlc_Condicion = String.Format("{0} AND {1} = {2}", vlc_Condicion, Modelo.V_OTM_CATEGORIA_MATERIALLST.ID_UBICACION_ADMINISTRA, Me.AutorizadoUbicacion.IdUbicacionAdministra)
        End If
