...
Partial Class Catalogos_Almacen_Lst_OT_TipoIncidente
...
#Region "Métodos"
    ...
    Private Sub CargarLista(pvc_Condicion As String, pvc_Orden As String, pvn_NumeroDePagina As Integer)
        ...
        Try
            ...
            vlo_DsDatos = vlo_Ws_OT_Catalogos.OTM_TIPO_INCIDENTE_ListarRegistrosLista(
    ...
    Private Sub Buscar(pvc_Condicion As String, pvc_Orden As String)
        ...
        If String.IsNullOrWhiteSpace(pvc_Orden) Then
            pvc_Orden = ObtenerExpresionDeOrdenamiento(Modelo.V_OTM_TIPO_INCIDENTELST.DESCRIPCION)
        End If
        ...
        Try
            ...
            vlo_EntDatosPaginacion =
                vlo_Ws_OT_Catalogos.OTM_TIPO_INCIDENTE_ObtenerDatosPaginacionVOtmTipoIncidentelst(
                     ConfigurationManager.AppSettings(ConstantesInternas.APP_KEY_USUARIO_APLICACION_WEB),
                     ConfigurationManager.AppSettings(ConstantesInternas.APP_KEY_CLAVE_APLICACION_WEB),
                     pvc_Condicion,
                     pvc_Orden,
...
#Region "Funciones"
    ...
    ''' </summary>
    ''' <param name="pvn_IdTipoIncidente"></param>
    ''' <returns>Si retorna un número mayor a 0 quiere decir que la operacion se realizo con éxito</returns>
    ''' <remarks></remarks>
    ''' <author>Carlos Gómez Ondoy</author>
    ''' <creationDate>04/08/2016</creationDate>
    ''' <changeLog></changeLog>
    Private Function Borrar(pvn_IdTipoIncidente As Integer) As Boolean
        Dim vlo_Ws_OT_Catalogos As Wsr_OT_Catalogos.Ws_OT_Catalogos
        ...
        vlo_EntOtmTipoIncidente.IdTipoIncidente = pvn_IdTipoIncidente

        Try
            Return vlo_Ws_OT_Catalogos.OTM_TIPO_INCIDENTE_BorrarRegistro(
                ConfigurationManager.AppSettings(ConstantesInternas.APP_KEY_USUARIO_APLICACION_WEB),
                ConfigurationManager.AppSettings(ConstantesInternas.APP_KEY_CLAVE_APLICACION_WEB),
    ...
    Private Function ObtenerCondicionBusqueda() As String
        ...
        If Not String.IsNullOrWhiteSpace(Me.txtDescripcion.Text.Trim) Then
            If String.IsNullOrWhiteSpace(vlc_Condicion) Then
                vlc_Condicion = String.Format("UPPER({0}) LIKE '%{1}%'", Modelo.V_OTM_TIPO_INCIDENTELST.DESCRIPCION, Me.txtDescripcion.Text.Trim.ToUpper)
            Else
                vlc_Condicion = String.Format("{0} AND UPPER({1}) LIKE '%{2}%'", vlc_Condicion, Modelo.V_OTM_TIPO_INCIDENTELST.DESCRIPCION, Me.txtDescripcion.Text.Trim.ToUpper)
            End If
        End If

        If Not String.IsNullOrWhiteSpace(Me.ddlFiltroEstado.SelectedValue) Then
            If String.IsNullOrWhiteSpace(vlc_Condicion) Then
                vlc_Condicion = String.Format("{0} = '{1}'", Modelo.V_OTM_TIPO_INCIDENTELST.ESTADO, Me.ddlFiltroEstado.SelectedValue)
            Else
                vlc_Condicion = String.Format("{0} AND {1} = '{2}'", vlc_Condicion, Modelo.V_OTM_TIPO_INCIDENTELST.ESTADO, Me.ddlFiltroEstado.SelectedValue)
            End If
        End If
