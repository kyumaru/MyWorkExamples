...
Partial Class Catalogos_Lst_OT_TipoLugarUbicacion
...
#Region "Eventos"
    Protected Sub Page_Load(sender As Object, e As EventArgs) Handles Me.Load
        ...
        Me.pnRpLugarUbicaciones.Dibujar()
    End Sub

    Protected Sub rpLugarUbicaciones_ItemDataBound(sender As Object, e As RepeaterItemEventArgs) Handles rpLugarUbicaciones.ItemDataBound
    ...
    Protected Sub pnRpLugarUbicaciones_CambioDePagina(pvn_PaginaSeleccionada As Integer) Handles pnRpLugarUbicaciones.CambioDePagina
        Try
    ...
    Protected Sub lnkRpLugarUbicaciones_Command(sender As Object, e As CommandEventArgs)
        Try
            CargarLista(ObtenerCondicionDeBusqueda, ObtenerSortExpression(e.CommandName), pnRpLugarUbicaciones.PaginaActualLista)
        Catch ex As Exception
            Dim vlo_ControlDeErrores As New ControlDeErrores
            vlo_ControlDeErrores.RegistrarExcepcion(ex, String.Empty)
        End Try
...
#Region "Metodos"
    ...
    Private Sub CargarEstadoLugarUbicacion() 'CondicionLugarUbicacion
    ...
    Private Sub CargarLista(pvc_Condicion As String, pvc_Orden As String, pvn_NumeroDePagina As Integer)
        ...
        Try
            vlo_DsDatos = vlo_Ws_OT_Catalogos.OTM_TIPO_LUGAR_UBICACION_ListarRegistrosLista(
    ...
    Private Sub Buscar(pvc_Condicion As String, pvc_Orden As String)
        ...
        If String.IsNullOrWhiteSpace(pvc_Orden) Then
            pvc_Orden = ObtenerSortExpression(Modelo.V_OTM_TIPO_LUGAR_UBICACIONLST.DESCRIPCION) 'lo pide el programa ordenar x isbn
        End If
        ...
        Try
            vlo_EntDatosPaginacion = vlo_Ws_OT_Catalogos.OTM_TIPO_LUGAR_UBICACION_ObtenerDatosPaginacionVOtmTipoLugarUbicacionlst(
                ConfigurationManager.AppSettings(ConstantesInternas.APP_KEY_USUARIO_APLICACION_WEB),
                ConfigurationManager.AppSettings(ConstantesInternas.APP_KEY_CLAVE_APLICACION_WEB),
                pvc_Condicion,
                pvc_Orden,
            ...
            If vlo_EntDatosPaginacion.TotalRegistros > 0 Then
                Me.pnRpLugarUbicaciones.TotalPaginasLista = vlo_EntDatosPaginacion.TotalPaginas
                Me.pnRpLugarUbicaciones.Dibujar()
                lblCantidadRegistro.Visible = True
                lblCantidadRegistro.Text = String.Format("Cantidad de lugares de ubicación: {0}", vlo_EntDatosPaginacion.TotalRegistros)
...
#Region "Funciones"
    ...
    ''' </summary>
    ''' <param name="pvc_Id_LugarUbicacion"></param>
    ''' <returns></returns>
    ''' <remarks></remarks>
    Private Function Borrar(pvc_Id_LugarUbicacion As String) As Boolean
        Dim vlo_Ws_OT_Catalogos As Ws_OT_Catalogos
        ...
        Try
            vlo_EntOtmTipoLugarUbicacion.IdTipoLugarUbicacion = pvc_Id_LugarUbicacion
            'vlo_EntTcmLibro.Usuario = New UsuarioActual().UserName 
            Return vlo_Ws_OT_Catalogos.OTM_TIPO_LUGAR_UBICACION_BorrarRegistro(
                ConfigurationManager.AppSettings(ConstantesInternas.APP_KEY_USUARIO_APLICACION_WEB),
                ConfigurationManager.AppSettings(ConstantesInternas.APP_KEY_CLAVE_APLICACION_WEB),
    ...
    Private Function ObtenerCondicionDeBusqueda() As String
        ...
        If Not String.IsNullOrWhiteSpace(Me.txtFiltroDescripcion.Text) Then
            If String.IsNullOrWhiteSpace(vlc_Condicion) Then
                ...
                vlc_Condicion = String.Format("UPPER({0}) LIKE '%{1}%'", Modelo.V_OTM_TIPO_LUGAR_UBICACIONLST.DESCRIPCION, Me.txtFiltroDescripcion.Text.Trim.ToUpper)
            Else
                '0 condicion original, 1 columna db y 2 valor busqueda
                vlc_Condicion = String.Format("{0} AND UPPER({1}) LIKE '%{2}&'", vlc_Condicion, Modelo.V_OTM_TIPO_LUGAR_UBICACIONLST.DESCRIPCION, Me.txtFiltroDescripcion.Text.Trim.ToUpper)
        ...
        If Not String.IsNullOrWhiteSpace(Me.ddlFiltroEstado.SelectedValue) Then
            If String.IsNullOrWhiteSpace(vlc_Condicion) Then
                '0 columna bd y 1 valor busqueda
                vlc_Condicion = String.Format("{0} = '{1}'", Modelo.V_OTM_TIPO_LUGAR_UBICACIONLST.ESTADO, Me.ddlFiltroEstado.SelectedValue)
            Else
                '0 condicion original, 1 columna db y 2 valor busqueda
                vlc_Condicion = String.Format("{0} AND {1} = '{2}'", vlc_Condicion, Modelo.V_OTM_TIPO_LUGAR_UBICACIONLST.ESTADO, Me.ddlFiltroEstado.SelectedValue)
