...
Partial Class Catalogos_Lst_OT_SectorTaller
...
#Region "Eventos"
    ...
    Protected Sub pnRpLugares_CambioDePagina(pvn_PaginaSeleccionada As Integer) Handles pnRpLugares.CambioDePagina
        Try
    ...
    Protected Sub lnkRpLugares_Command(sender As Object, e As CommandEventArgs)
        Try
            CargarLista(Me.Condicion, ObtenerSortExpression(e.CommandName), pnRpLugares.PaginaActualLista)
        Catch ex As Exception
            Dim vlo_ControlDeErrores As New ControlDeErrores
            vlo_ControlDeErrores.RegistrarExcepcion(ex, String.Empty)
        End Try
...
#Region "Metodos"
    ...
    Private Sub Buscar(pvc_Condicion As String, pvc_Orden As String)
        ...
        If String.IsNullOrWhiteSpace(pvc_Orden) Then
            pvc_Orden = ObtenerSortExpression(Modelo.V_OTM_SECTOR_TALLERLST.NOMBRE)
        End If
        ...
        Try
            vlo_EntDatosPaginacion = vlo_Ws_OT_Catalogos.OTM_SECTOR_TALLER_ObtenerDatosPaginacionVOtmSectorTallerlst(
                ConfigurationManager.AppSettings(ConstantesInternas.APP_KEY_USUARIO_APLICACION_WEB),
                ConfigurationManager.AppSettings(ConstantesInternas.APP_KEY_CLAVE_APLICACION_WEB),
                pvc_Condicion,
                pvc_Orden,
            ...
            If vlo_EntDatosPaginacion.TotalRegistros > 0 Then
                ...
                lblCantidadRegistro.Text = String.Format("Cantidad de Sectores y Talleres: {0}", vlo_EntDatosPaginacion.TotalRegistros)
...
#Region "Funciones"
    ...
    ''' </summary>
    ''' <param name="pvc_Id_SectorTaller"></param>
    ''' <returns></returns>
    ''' <remarks></remarks>
    Private Function Borrar(pvc_Id_SectorTaller As String) As Boolean
        Dim vlo_Ws_OT_Catalogos As Ws_OT_Catalogos
        ...
        Try
            vlo_Clave = pvc_Id_SectorTaller.Split("_")

            vlo_EntOtmSectorTaller.IdSectorTaller = vlo_Clave(0)

            Roles.RemoveUserFromRole(vlo_Clave(1), Utilerias.OrdenesDeTrabajo.Constantes.ROL_COORDINADOR)

            If vlo_Clave(2) <> "" Then
                Roles.RemoveUserFromRole(vlo_Clave(2), Utilerias.OrdenesDeTrabajo.Constantes.ROL_COORDINADOR)
            End If

            'vlo_EntTcmLibro.Usuario = New UsuarioActual().UserName 
            Return vlo_Ws_OT_Catalogos.OTM_SECTOR_TALLER_BorrarRegistro(
                ConfigurationManager.AppSettings(ConstantesInternas.APP_KEY_USUARIO_APLICACION_WEB),
                ConfigurationManager.AppSettings(ConstantesInternas.APP_KEY_CLAVE_APLICACION_WEB),
    ...
    Private Function ObtenerCondicionDeBusqueda() As String
        ...
        If Not String.IsNullOrWhiteSpace(Me.txtFiltroNombre.Text) Then
            If String.IsNullOrWhiteSpace(vlc_Condicion) Then
                ...
                vlc_Condicion = String.Format("UPPER({0}) LIKE '%{1}%' AND {2} = {3}", Modelo.V_OTM_SECTOR_TALLER.NOMBRE, Me.txtFiltroNombre.Text.Trim.ToUpper, Modelo.V_OTM_SECTOR_TALLER.ID_UBICACION_ADMINISTRA, Me.UnidadAdministra)
            Else
                '0 condicion original, 1 columna db y 2 valor busqueda
                vlc_Condicion = String.Format("{0} AND UPPER({1}) LIKE '%{2}%' AND {3} = {4}", vlc_Condicion, Modelo.V_OTM_SECTOR_TALLER.NOMBRE, Me.txtFiltroNombre.Text.Trim.ToUpper, Modelo.V_OTM_SECTOR_TALLER.ID_UBICACION_ADMINISTRA, Me.UnidadAdministra)

            End If
        End If

        If Not String.IsNullOrWhiteSpace(Me.txtFiltroCoordinador.Text) Then
            If String.IsNullOrWhiteSpace(vlc_Condicion) Then
                '0 columna bd y 1 valor busqueda
                vlc_Condicion = String.Format("{0} LIKE '%{1}%' AND {2} = {3}", Modelo.V_OTM_SECTOR_TALLERLST.CEDULA_COORDINADOR, Me.txtFiltroCoordinador.Text, Modelo.V_OTM_SECTOR_TALLER.ID_UBICACION_ADMINISTRA, Me.UnidadAdministra)
            Else
                '0 condicion original, 1 columna db y 2 valor busqueda
                vlc_Condicion = String.Format("{0} AND {1} LIKE '%{2}%' AND {3} = {4}", vlc_Condicion, Modelo.V_OTM_SECTOR_TALLERLST.CEDULA_COORDINADOR, Me.txtFiltroCoordinador.Text, Modelo.V_OTM_SECTOR_TALLER.ID_UBICACION_ADMINISTRA, Me.UnidadAdministra)
        ...
        If Not String.IsNullOrWhiteSpace(Me.ddlFiltroEstado.SelectedValue) Then
            If String.IsNullOrWhiteSpace(vlc_Condicion) Then
                '0 columna bd y 1 valor busqueda
                vlc_Condicion = String.Format("{0} = '{1}' AND {2} = {3}", Modelo.V_OTM_SECTOR_TALLER.ESTADO, Me.ddlFiltroEstado.SelectedValue, Modelo.V_OTM_SECTOR_TALLER.ID_UBICACION_ADMINISTRA, Me.UnidadAdministra)
            Else
                '0 condicion original, 1 columna db y 2 valor busqueda
                vlc_Condicion = String.Format("{0} AND {1} = '{2}' AND {3} = {4}", vlc_Condicion, Modelo.V_OTM_SECTOR_TALLER.ESTADO, Me.ddlFiltroEstado.SelectedValue, Modelo.V_OTM_SECTOR_TALLER.ID_UBICACION_ADMINISTRA, Me.UnidadAdministra)
