...
Partial Class Catalogos_Lst_OT_TipoOrdenTrabajo
...
#Region "Eventos"
    ...
    Protected Sub rpTipoOrdenes_ItemDataBound(sender As Object, e As RepeaterItemEventArgs) Handles rpTipoOrdenes.ItemDataBound
    ...
    Protected Sub pnRpTipoOrdenes_CambioDePagina(pvn_PaginaSeleccionada As Integer) Handles pnRpTipoOrdenes.CambioDePagina
        Try
    ...
    Protected Sub lnkRpTipoOrdenes_Command(sender As Object, e As CommandEventArgs)
        Try
            CargarLista(ObtenerCondicionDeBusqueda, ObtenerSortExpression(e.CommandName), pnRpTipoOrdenes.PaginaActualLista)
        Catch ex As Exception
            Dim vlo_ControlDeErrores As New ControlDeErrores
            vlo_ControlDeErrores.RegistrarExcepcion(ex, String.Empty)
        End Try
...
#Region "Metodos"
    ...
    Private Sub Buscar(pvc_Condicion As String, pvc_Orden As String)
        ...
        If String.IsNullOrWhiteSpace(pvc_Orden) Then
            pvc_Orden = ObtenerSortExpression(Modelo.V_OTC_TIPO_ORDEN_TRABAJOLST.DESCRIPCION) 'lo pide el programa ordenar x isbn
        End If
        ...
        Try
            vlo_EntDatosPaginacion = vlo_Ws_OT_Catalogos.OTC_TIPO_ORDEN_TRABAJO_ObtenerDatosPaginacionVOtcTipoOrdenTrabajolst(
                ConfigurationManager.AppSettings(ConstantesInternas.APP_KEY_USUARIO_APLICACION_WEB),
                ConfigurationManager.AppSettings(ConstantesInternas.APP_KEY_CLAVE_APLICACION_WEB),
                pvc_Condicion,
                pvc_Orden,
            ...
            If vlo_EntDatosPaginacion.TotalRegistros > 0 Then
                Me.pnRpTipoOrdenes.TotalPaginasLista = vlo_EntDatosPaginacion.TotalPaginas
                Me.pnRpTipoOrdenes.Dibujar()
                lblCantidadRegistro.Visible = True
                lblCantidadRegistro.Text = String.Format("Cantidad de tipos de ordenes: {0}", vlo_EntDatosPaginacion.TotalRegistros)
...
#Region "Funciones"
    ...
    ''' </summary>
    ''' <param name="pvc_Id_TipoOrden"></param>
    ''' <returns></returns>
    ''' <remarks></remarks>
    Private Function Borrar(pvc_Id_TipoOrden As String) As Boolean
        Dim vlo_Ws_OT_Catalogos As Ws_OT_Catalogos
        ...
        Try
            vlo_EntOtcTipoOrdenTrabajo.TipoOrdenTrabajo = pvc_Id_TipoOrden
            'vlo_EntTcmLibro.Usuario = New UsuarioActual().UserName 
            Return vlo_Ws_OT_Catalogos.OTC_TIPO_ORDEN_TRABAJO_BorrarRegistro(
                ConfigurationManager.AppSettings(ConstantesInternas.APP_KEY_USUARIO_APLICACION_WEB),
                ConfigurationManager.AppSettings(ConstantesInternas.APP_KEY_CLAVE_APLICACION_WEB),
    ...
    Private Function ObtenerCondicionDeBusqueda() As String
        ...
        If Not String.IsNullOrWhiteSpace(Me.txtFiltroDescripcion.Text) Then
            If String.IsNullOrWhiteSpace(vlc_Condicion) Then
                ...
                vlc_Condicion = String.Format("UPPER({0}) LIKE '%{1}%'", Modelo.V_OTC_TIPO_ORDEN_TRABAJO.DESCRIPCION, Me.txtFiltroDescripcion.Text.Trim.ToUpper)
            Else
                '0 condicion original, 1 columna db y 2 valor busqueda
                vlc_Condicion = String.Format("{0} AND UPPER({1}) LIKE '{2}'", vlc_Condicion, Modelo.V_OTC_TIPO_ORDEN_TRABAJO.DESCRIPCION, Me.txtFiltroDescripcion.Text.Trim.ToUpper)
