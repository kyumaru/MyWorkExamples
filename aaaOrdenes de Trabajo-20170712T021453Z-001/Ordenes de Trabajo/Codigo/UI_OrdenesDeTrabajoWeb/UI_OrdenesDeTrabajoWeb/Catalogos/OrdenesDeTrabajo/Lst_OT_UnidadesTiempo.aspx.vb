...
Partial Class Catalogos_Lst_OT_UnidadesTiempo
...
#Region "Eventos"
    ...
    ''' <author>César Bermúdez García</author>
    ''' <creationDate>15/1/2016</creationDate>
    Protected Sub rpUnidadesTiempo_ItemDataBound(sender As Object, e As RepeaterItemEventArgs) Handles rpUnidadesTiempo.ItemDataBound
    ...
    Protected Sub lnkRPUnidadesTiempo_Command(sender As Object, e As CommandEventArgs)
        Try
            CargarLista(ObtenerCondicionDeBusqueda, ObtenerSortExpression(e.CommandName), pnRpUnidadesTiempo.PaginaActualLista)
        Catch ex As Exception
            Dim vlo_ControlDeErrores As New ControlDeErrores
            vlo_ControlDeErrores.RegistrarExcepcion(ex, String.Empty)
        End Try
    ...
    Protected Sub pnRpUnidadesTiempo_CambioDePagina(pvn_PaginaSeleccionada As Integer) Handles pnRpUnidadesTiempo.CambioDePagina
        Try
...
#Region "Metodos"
    ...
    Private Sub Buscar(pvc_Condicion As String, pvc_Orden As String)
        ...
        Try
            vlo_EndDatosPaginacion = vlo_Ws_OT_Catalogos.OTM_UNIDAD_TIEMPO_ObtenerDatosPaginacionVOtmUnidadTiempo(
                ConfigurationManager.AppSettings(ConstantesInternas.APP_KEY_USUARIO_APLICACION_WEB),
                ConfigurationManager.AppSettings(ConstantesInternas.APP_KEY_CLAVE_APLICACION_WEB),
            ...
            If vlo_EndDatosPaginacion.TotalRegistros > 0 Then
                Me.pnRpUnidadesTiempo.TotalPaginasLista = vlo_EndDatosPaginacion.TotalPaginas
                Me.pnRpUnidadesTiempo.Dibujar()
    ...
    Protected Sub ibBorrar_Click(sender As Object, e As ImageClickEventArgs)
        ...
        Dim vlc_IdUnidadTiempo As String

        Try
            vlo_IbBorrar = CType(sender, ImageButton)
            vlc_IdUnidadTiempo = vlo_IbBorrar.CommandArgument

            If Borrar(vlc_IdUnidadTiempo) Then
...
#Region "Funciones"
    ...
    Private Function Borrar(pvc_Id As String) As Boolean
        ...
        Try
            vlo_EntOtmUnidadTiempo.IdUnidadTiempo = pvc_Id

            Return vlo_Ws_OT_Catalogos.OTM_UNIDAD_TIEMPO_BorrarRegistro(
                ConfigurationManager.AppSettings(ConstantesInternas.APP_KEY_USUARIO_APLICACION_WEB),
                ConfigurationManager.AppSettings(ConstantesInternas.APP_KEY_CLAVE_APLICACION_WEB),
        ...
        If Not String.IsNullOrWhiteSpace(Me.txtFiltroDescripcion.Text) Then
            If String.IsNullOrWhiteSpace(vlc_Condicion) Then
                ...
                vlc_Condicion = String.Format("UPPER({0}) LIKE '%{1}%'",
                                            Modelo.OTM_UNIDAD_TIEMPO.DESCRIPCION,
        ...
        If Not String.IsNullOrWhiteSpace(Me.ddlFiltroEstado.SelectedValue) Then
            If String.IsNullOrWhiteSpace(vlc_Condicion) Then
                ...
                vlc_Condicion = String.Format("{0} = '{1}'",
                                            Modelo.OTM_UNIDAD_TIEMPO.ESTADO,
