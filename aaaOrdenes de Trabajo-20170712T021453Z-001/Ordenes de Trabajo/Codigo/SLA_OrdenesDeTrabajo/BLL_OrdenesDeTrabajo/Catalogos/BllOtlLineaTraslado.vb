...
Namespace ORDENES_TRABAJO.LogicaNegocio.Catalogos
    Public Class BllOtlLineaTraslado
#Region "Atributos"
...
#Region "Funciones"
        ''' <summary>
        ''' Permite agregar un registro en la tabla OTL_LINEA_TRASLADO, no sin antes aplicar la validación de la llave primaria o alterna según corresponda
        ''' </summary>
        ...
        ''' <changeLog></changeLog>
        Public Function InsertarRegistro(ByVal pvo_Registro As EntOtlLineaTraslado) As Integer
            ...
            Try
                ...
                vln_Resultado = vlo_DalOtlLineaTraslado.InsertarRegistro(pvo_Registro)
        ...
        ''' <param name="pvn_IdAlmacen">Llave primaria de la tabla otm_almacen_bodega que se asocia con la secuencia sq_id_almacen_bodega</param>
        ...
        ''' <creationDate>09/08/2016 09:07:05 a.m.</creationDate>
        ''' <changeLog></changeLog>
        Private Function ObtenerRegistroPorLlavePrimaria(pvn_Anno As Integer, pvn_IdSolicitudTraslado As Integer, pvn_IdUbicacion As Integer, pvn_IdAlmacen As Integer, pvn_IdMaterial As Integer) As EntOtlLineaTraslado
            Dim vlo_Conexion As ConexionOracle
            Dim vlb_LiberarConexion As Boolean
            Dim vlo_DalOtlLineaTraslado As DalOtlLineaTraslado
            ...
            Try
                ...
                vlo_DalOtlLineaTraslado = New DalOtlLineaTraslado(vlo_Conexion)
                Return vlo_DalOtlLineaTraslado.ObtenerRegistro(String.Format("{0} = {1} AND {2} = {3} AND {4} = {5} AND {6} = {7} AND {8} = {9}", Utilerias.OrdenesDeTrabajo.Modelo.OTL_LINEA_TRASLADO.ANNO, pvn_Anno, Utilerias.OrdenesDeTrabajo.Modelo.OTL_LINEA_TRASLADO.ID_SOLICITUD_TRASLADO, pvn_IdSolicitudTraslado, Utilerias.OrdenesDeTrabajo.Modelo.OTL_LINEA_TRASLADO.ID_UBICACION, pvn_IdUbicacion, Utilerias.OrdenesDeTrabajo.Modelo.OTL_LINEA_TRASLADO.ID_ALMACEN, pvn_IdAlmacen, Utilerias.OrdenesDeTrabajo.Modelo.OTL_LINEA_TRASLADO.ID_MATERIAL, pvn_IdMaterial))
            Catch vlo_Excepcion As Exception
                Throw
            Finally
                If vlb_LiberarConexion Then
