...
Namespace OrdenesDeTrabajo.LogicaNegocio.OrdenesDeTrabajo
	Public Class BllOttAdjuntoCotizacion
#Region "Atributos"
...
#Region "Funciones"
		''' <summary>
		''' Permite agregar un registro en la tabla OTT_ADJUNTO_COTIZACION, no sin antes aplicar la validación de la llave primaria o alterna según corresponda
		''' </summary>
		...
		''' <changeLog></changeLog>
		Public Function InsertarRegistro(ByVal pvo_Registro As EntOttAdjuntoCotizacion) As Integer
			...
			Try
				...
				vln_Resultado = vlo_DalOttAdjuntoCotizacion.InsertarRegistro(pvo_Registro)
		...
		''' <creationDate>25/08/2016 12:17:45 p.m.</creationDate>
		''' <changeLog></changeLog>
		Private Function ObtenerRegistroPorLlavePrimaria(pvc_Identificacion As String, pvn_IdUbicacion As Integer, pvn_IdViaCompraContrato As Integer, pvn_Anno As Integer, pvn_NumeroGestion As Integer) As EntOttAdjuntoCotizacion
			Dim vlo_Conexion As ConexionOracle
			Dim vlb_LiberarConexion As Boolean
			Dim vlo_DalOttAdjuntoCotizacion As DalOttAdjuntoCotizacion
			...
			Try
				...
				vlo_DalOttAdjuntoCotizacion = New DalOttAdjuntoCotizacion(vlo_Conexion)
				Return vlo_DalOttAdjuntoCotizacion.ObtenerRegistro(String.Format("UPPER({0}) = '{1}' AND {2} = {3} AND {4} = {5} AND {6} = {7} AND {8} = {9}", Modelo.OTT_ADJUNTO_COTIZACION.IDENTIFICACION, pvc_Identificacion.ToUpper(), Modelo.OTT_ADJUNTO_COTIZACION.ID_UBICACION, pvn_IdUbicacion, Modelo.OTT_ADJUNTO_COTIZACION.ID_VIA_COMPRA_CONTRATO, pvn_IdViaCompraContrato, Modelo.OTT_ADJUNTO_COTIZACION.ANNO, pvn_Anno, Modelo.OTT_ADJUNTO_COTIZACION.NUMERO_GESTION, pvn_NumeroGestion))
			Catch vlo_Excepcion As Exception
				Throw
			Finally
				If vlb_LiberarConexion Then
