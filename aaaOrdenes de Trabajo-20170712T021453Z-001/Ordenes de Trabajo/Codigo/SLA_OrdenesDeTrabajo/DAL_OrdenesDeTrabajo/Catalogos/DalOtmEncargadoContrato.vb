...
Namespace OrdenesDeTrabajo.AccesoDatos.Catalogos
	Public Class DalOtmEncargadoContrato
		Inherits DalBase
...
#Region "Funciones"
	...
	Public Overrides Function InsertarRegistro(ByVal pvo_Registro As EntBase) As Integer
		...
		Dim vlo_RegistroInterno As EntOtmEncargadoContrato
		Dim vln_Resultado As Integer

		Try
			vln_Resultado = -1
			vlo_RegistroInterno = CType(pvo_Registro, EntOtmEncargadoContrato)
			...
			vlc_Sentencia = "prI_OTM_ENCARGADO_CONTRATO"
	...
	Public Overrides Function BorrarRegistro(ByVal pvo_Registro As EntBase) As Integer
		...
		Try
			...
			vlc_Sentencia = "prD_OTM_ENCARGADO_CONTRATO"
	...
	''' <summary>
	''' Permite modificar un registro en la tabla OTM_ENCARGADO_CONTRATO
	''' </summary>
	''' <param name="pvo_Registro">Entidad a procesar</param>
	''' <returns>Mayor a cero: El proceso ha sido exitoso. Menor a cero: El proceso ha fallado</returns>
	''' <author>Generador de Código basado en objetos Oracle</author>
	...
	Public Overrides Function ModificarRegistro(ByVal pvo_Registro As EntBase) As Integer
		...
		Try
			...
			vlc_Sentencia = "prU_OTM_ENCARGADO_CONTRATO"
	...
	Public Overrides Function ObtenerRegistro(ByVal pvc_Condicion As String) As EntBase
		...
		Dim vlo_Resultado As New EntOtmEncargadoContrato

		Try
			vlo_DsDatos = ListarRegistros(pvc_Condicion, String.Empty, False, 0, 0)
			If vlo_DsDatos IsNot Nothing AndAlso vlo_DsDatos.Tables(0).Rows.Count > 0 Then
				vlo_MapeoEntidad = New List(Of MapeoSimple)
				vlo_MapeoEntidad.Add(New MapeoSimple(OTM_ENCARGADO_CONTRATO.NUM_EMPLEADO, "NumEmpleado"))
				vlo_MapeoEntidad.Add(New MapeoSimple(OTM_ENCARGADO_CONTRATO.ID_UBICACION, "IdUbicacion"))
				vlo_MapeoEntidad.Add(New MapeoSimple(OTM_ENCARGADO_CONTRATO.ESTADO, "Estado"))
				vlo_MapeoEntidad.Add(New MapeoSimple(OTM_ENCARGADO_CONTRATO.USUARIO, "Usuario"))
				vlo_MapeoEntidad.Add(New MapeoSimple(OTM_ENCARGADO_CONTRATO.TIME_STAMP, "TimeStamp"))

				vlo_Resultado = vlo_DsDatos.Tables(0).Rows(0).ToEntity(Of EntOtmEncargadoContrato)(vlo_MapeoEntidad)
	...
	Public Overrides Function ListarRegistros(pvc_Condicion As String, pvc_Orden As String, pvb_Paginar As Boolean, pvn_NumeroPagina As Integer, pvn_TamanoPagina As Integer) As DataSet
		...
		Try
			...
			vlo_DsDatos = Me.EjecutarVista(vlo_Conexion, OTM_ENCARGADO_CONTRATO.Name, "V_OTM_ENCARGADO_CONTRATO", pvc_Condicion, pvc_Orden, pvb_Paginar, pvn_NumeroPagina, pvn_TamanoPagina)
	...
	''' <creationDate>14/04/2016 12:03:08 p.m.</creationDate>
	''' <changeLog></changeLog>
	Public Function ObtenerDatosPaginacionVOtmEncargadoContrato(pvc_Condicion As String, pvc_Orden As String, pvn_TamanoPagina As Integer) As EntDatosPaginacion
		Dim vlo_Conexion As DbBase
		Dim vlb_DisposeConexion As Boolean
		...
		Try
			...
			vlo_Resultado = Me.ObtenerDatosPaginacionVista(vlo_Conexion, "V_OTM_ENCARGADO_CONTRATO", pvc_Condicion, pvc_Orden, pvn_TamanoPagina)
	...
	Public Overrides Function ListarRegistrosLista(pvc_Condicion As String, pvc_Orden As String, pvb_Paginar As Boolean, pvn_NumeroPagina As Integer, pvn_TamanoPagina As Integer) As DataSet
		...
		Try
			...
			vlo_DsDatos = Me.EjecutarVista(vlo_Conexion, OTM_ENCARGADO_CONTRATO.Name, "V_OTM_ENCARGADO_CONTRATOLst", pvc_Condicion, pvc_Orden, pvb_Paginar, pvn_NumeroPagina, pvn_TamanoPagina)
	...
	''' <creationDate>14/04/2016 12:03:08 p.m.</creationDate>
	''' <changeLog></changeLog>
	Public Function ObtenerDatosPaginacionVOtmEncargadoContratolst(pvc_Condicion As String, pvc_Orden As String, pvn_TamanoPagina As Integer) As EntDatosPaginacion
		Dim vlo_Conexion As DbBase
		Dim vlb_DisposeConexion As Boolean
		Dim vlo_Resultado As EntDatosPaginacion
		...
		Try
			...
			vlo_Resultado = Me.ObtenerDatosPaginacionVista(vlo_Conexion, "V_OTM_ENCARGADO_CONTRATOLst", pvc_Condicion, pvc_Orden, pvn_TamanoPagina)
