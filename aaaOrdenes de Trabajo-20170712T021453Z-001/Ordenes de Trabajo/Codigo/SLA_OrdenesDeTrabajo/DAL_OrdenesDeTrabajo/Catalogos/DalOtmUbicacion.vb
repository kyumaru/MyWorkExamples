...
#Region "Funciones"
	...
	Public Overrides Function InsertarRegistro(ByVal pvo_Registro As EntBase) As Integer
		...
		Dim vlo_RegistroInterno As EntOtmUbicacion
		Dim vln_Resultado As Integer

		Try
			vln_Resultado = -1
			vlo_RegistroInterno = CType(pvo_Registro, EntOtmUbicacion)
			...
			vlc_Sentencia = "prI_OTM_UBICACION"

			vlo_Conexion.SetParameter("pvc_Descripcion", OracleDbType.Varchar2, vlo_RegistroInterno.Descripcion)
			vlo_Conexion.SetParameter("pvn_PerteneceASede", OracleDbType.Int32, vlo_RegistroInterno.PerteneceASede)
			vlo_Conexion.SetParameter("pvc_Estado", OracleDbType.Varchar2, vlo_RegistroInterno.Estado)
			vlo_Conexion.SetParameter("prn_IdUbicacion", OracleDbType.Int32, vln_Resultado, ParameterDirection.Output)
			vlo_Conexion.EjecutarSentencia(vlc_Sentencia, CommandType.StoredProcedure)
			vln_Resultado = CType(vlo_Conexion.GetParameterValue("prn_IdUbicacion"), Integer)
	...
	Public Overrides Function BorrarRegistro(ByVal pvo_Registro As EntBase) As Integer
		...
		Try
			...
			vlc_Sentencia = "prD_OTM_UBICACION"
	...
	''' <summary>
	''' Permite modificar un registro en la tabla OTM_UBICACION
	''' </summary>
	''' <param name="pvo_Registro">Entidad a procesar</param>
	''' <returns>Mayor a cero: El proceso ha sido exitoso. Menor a cero: El proceso ha fallado</returns>
	''' <author>Generador de Código basado en objetos Oracle</author>
	...
	Public Overrides Function ModificarRegistro(ByVal pvo_Registro As EntBase) As Integer
		...
		Try
			...
			vlc_Sentencia = "prU_OTM_UBICACION"
	...
	Public Overrides Function ObtenerRegistro(ByVal pvc_Condicion As String) As EntBase
		...
		Dim vlo_Resultado As New EntOtmUbicacion

		Try
			vlo_DsDatos = ListarRegistros(pvc_Condicion, String.Empty, False, 0, 0)
			If vlo_DsDatos IsNot Nothing AndAlso vlo_DsDatos.Tables(0).Rows.Count > 0 Then
				vlo_MapeoEntidad = New List(Of MapeoSimple)
				vlo_MapeoEntidad.Add(New MapeoSimple(OTM_UBICACION.DESCRIPCION, "Descripcion"))
				vlo_MapeoEntidad.Add(New MapeoSimple(OTM_UBICACION.PERTENECE_A_SEDE, "PerteneceASede"))
				vlo_MapeoEntidad.Add(New MapeoSimple(OTM_UBICACION.ESTADO, "Estado"))
				vlo_MapeoEntidad.Add(New MapeoSimple(OTM_UBICACION.TIME_STAMP, "TimeStamp"))
				vlo_MapeoEntidad.Add(New MapeoSimple(OTM_UBICACION.ID_UBICACION, "IdUbicacion"))

				vlo_Resultado = vlo_DsDatos.Tables(0).Rows(0).ToEntity(Of EntOtmUbicacion)(vlo_MapeoEntidad)
	...
	Public Overrides Function ListarRegistros(pvc_Condicion As String, pvc_Orden As String, pvb_Paginar As Boolean, pvn_NumeroPagina As Integer, pvn_TamanoPagina As Integer) As DataSet
		...
		Try
			...
			vlo_DsDatos = Me.EjecutarVista(vlo_Conexion, OTM_UBICACION.Name, "V_OTM_UBICACION", pvc_Condicion, pvc_Orden, pvb_Paginar, pvn_NumeroPagina, pvn_TamanoPagina)
	...
	''' <creationDate>01/09/2015 05:07:51 p.m.</creationDate>
	''' <changeLog></changeLog>
	Public Function ObtenerDatosPaginacionVOtmUbicacion(pvc_Condicion As String, pvc_Orden As String, pvn_TamanoPagina As Integer) As EntDatosPaginacion
		Dim vlo_Conexion As DbBase
		Dim vlb_DisposeConexion As Boolean
		...
		Try
			...
			vlo_Resultado = Me.ObtenerDatosPaginacionVista(vlo_Conexion, "V_OTM_UBICACION", pvc_Condicion, pvc_Orden, pvn_TamanoPagina)
	...
	Public Overrides Function ListarRegistrosLista(pvc_Condicion As String, pvc_Orden As String, pvb_Paginar As Boolean, pvn_NumeroPagina As Integer, pvn_TamanoPagina As Integer) As DataSet
		...
		Try
			...
			vlo_DsDatos = Me.EjecutarVista(vlo_Conexion, OTM_UBICACION.Name, "V_OTM_UBICACIONLst", pvc_Condicion, pvc_Orden, pvb_Paginar, pvn_NumeroPagina, pvn_TamanoPagina)
	...
	''' <creationDate>01/09/2015 05:07:51 p.m.</creationDate>
	''' <changeLog></changeLog>
	Public Function ObtenerDatosPaginacionVOtmUbicacionlst(pvc_Condicion As String, pvc_Orden As String, pvn_TamanoPagina As Integer) As EntDatosPaginacion
		Dim vlo_Conexion As DbBase
		Dim vlb_DisposeConexion As Boolean
		Dim vlo_Resultado As EntDatosPaginacion
		...
		Try
			...
			vlo_Resultado = Me.ObtenerDatosPaginacionVista(vlo_Conexion, "V_OTM_UBICACIONLst", pvc_Condicion, pvc_Orden, pvn_TamanoPagina)
