...
Namespace OrdenesDeTrabajo.AccesoDatos.Catalogos
	Public Class DalOtfPeriodoCierre
		Inherits DalBase
...
#Region "Funciones"
	...
	''' <creationDate>18/01/2016 02:57:04 p.m.</creationDate>
	''' <changeLog></changeLog>
	Public Overrides Function InsertarRegistro(ByVal pvo_Registro As EntBase) As Integer
		...
		Dim vlo_RegistroInterno As EntOtfPeriodoCierre
		Dim vln_Resultado As Integer

		Try
			vln_Resultado = -1
			vlo_RegistroInterno = CType(pvo_Registro, EntOtfPeriodoCierre)
			...
			vlc_Sentencia = "prI_OTF_PERIODO_CIERRE"

			vlo_Conexion.SetParameter("pvn_IdUbicacion", OracleDbType.Int32, vlo_RegistroInterno.IdUbicacion)
			vlo_Conexion.SetParameter("pvc_UnidadCierre", OracleDbType.Varchar2, vlo_RegistroInterno.UnidadCierre)
			vlo_Conexion.SetParameter("pvd_FechaInicioCierre", OracleDbType.Date, vlo_RegistroInterno.FechaInicioCierre)
			vlo_Conexion.SetParameter("pvd_FechaFinCierre", OracleDbType.Date, vlo_RegistroInterno.FechaFinCierre)
	...
	Public Overrides Function BorrarRegistro(ByVal pvo_Registro As EntBase) As Integer
		...
		Try
			...
			vlc_Sentencia = "prD_OTF_PERIODO_CIERRE"

			vlo_Conexion.SetParameter("pvn_IdPeriodoCierre", OracleDbType.Int32, vlo_RegistroInterno.IdPeriodoCierre)
	...
	''' <summary>
	''' Permite modificar un registro en la tabla OTF_PERIODO_CIERRE
	''' </summary>
	''' <param name="pvo_Registro">Entidad a procesar</param>
	''' <returns>Mayor a cero: El proceso ha sido exitoso. Menor a cero: El proceso ha fallado</returns>
	''' <author>Generador de Código basado en objetos Oracle</author>
	...
	Public Overrides Function ModificarRegistro(ByVal pvo_Registro As EntBase) As Integer
		...
		Try
			...
			vlc_Sentencia = "prU_OTF_PERIODO_CIERRE"
	...
	Public Overrides Function ObtenerRegistro(ByVal pvc_Condicion As String) As EntBase
		...
		Dim vlo_Resultado As New EntOtfPeriodoCierre

		Try
			vlo_DsDatos = ListarRegistros(pvc_Condicion, String.Empty, False, 0, 0)
			If vlo_DsDatos IsNot Nothing AndAlso vlo_DsDatos.Tables(0).Rows.Count > 0 Then
				vlo_MapeoEntidad = New List(Of MapeoSimple)
				vlo_MapeoEntidad.Add(New MapeoSimple(OTF_PERIODO_CIERRE.ID_PERIODO_CIERRE, "IdPeriodoCierre"))
				vlo_MapeoEntidad.Add(New MapeoSimple(OTF_PERIODO_CIERRE.ID_UBICACION, "IdUbicacion"))
				vlo_MapeoEntidad.Add(New MapeoSimple(OTF_PERIODO_CIERRE.UNIDAD_CIERRE, "UnidadCierre"))
				vlo_MapeoEntidad.Add(New MapeoSimple(OTF_PERIODO_CIERRE.FECHA_INICIO_CIERRE, "FechaInicioCierre"))
				vlo_MapeoEntidad.Add(New MapeoSimple(OTF_PERIODO_CIERRE.FECHA_FIN_CIERRE, "FechaFinCierre"))
				vlo_MapeoEntidad.Add(New MapeoSimple(OTF_PERIODO_CIERRE.TIME_STAMP, "TimeStamp"))
				vlo_MapeoEntidad.Add(New MapeoSimple(OTF_PERIODO_CIERRE.USUARIO, "Usuario"))

				vlo_Resultado = vlo_DsDatos.Tables(0).Rows(0).ToEntity(Of EntOtfPeriodoCierre)(vlo_MapeoEntidad)
	...
	Public Overrides Function ListarRegistros(pvc_Condicion As String, pvc_Orden As String, pvb_Paginar As Boolean, pvn_NumeroPagina As Integer, pvn_TamanoPagina As Integer) As DataSet
		...
		Try
			...
			vlo_DsDatos = Me.EjecutarVista(vlo_Conexion, OTF_PERIODO_CIERRE.Name, "V_OTF_PERIODO_CIERRE", pvc_Condicion, pvc_Orden, pvb_Paginar, pvn_NumeroPagina, pvn_TamanoPagina)
	...
	''' <creationDate>18/01/2016 02:57:04 p.m.</creationDate>
	''' <changeLog></changeLog>
	Public Function ObtenerDatosPaginacionVOtfPeriodoCierre(pvc_Condicion As String, pvc_Orden As String, pvn_TamanoPagina As Integer) As EntDatosPaginacion
		Dim vlo_Conexion As DbBase
		Dim vlb_DisposeConexion As Boolean
		...
		Try
			...
			vlo_Resultado = Me.ObtenerDatosPaginacionVista(vlo_Conexion, "V_OTF_PERIODO_CIERRE", pvc_Condicion, pvc_Orden, pvn_TamanoPagina)
	...
	Public Overrides Function ListarRegistrosLista(pvc_Condicion As String, pvc_Orden As String, pvb_Paginar As Boolean, pvn_NumeroPagina As Integer, pvn_TamanoPagina As Integer) As DataSet
		...
		Try
			...
			vlo_DsDatos = Me.EjecutarVista(vlo_Conexion, OTF_PERIODO_CIERRE.Name, "V_OTF_PERIODO_CIERRELst", pvc_Condicion, pvc_Orden, pvb_Paginar, pvn_NumeroPagina, pvn_TamanoPagina)
	...
	''' <creationDate>18/01/2016 02:57:04 p.m.</creationDate>
	''' <changeLog></changeLog>
	Public Function ObtenerDatosPaginacionVOtfPeriodoCierrelst(pvc_Condicion As String, pvc_Orden As String, pvn_TamanoPagina As Integer) As EntDatosPaginacion
		Dim vlo_Conexion As DbBase
		Dim vlb_DisposeConexion As Boolean
		Dim vlo_Resultado As EntDatosPaginacion
		...
		Try
			...
			vlo_Resultado = Me.ObtenerDatosPaginacionVista(vlo_Conexion, "V_OTF_PERIODO_CIERRELst", pvc_Condicion, pvc_Orden, pvn_TamanoPagina)
